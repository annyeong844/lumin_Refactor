...
13. **응집도 있는 파일 소유권:** 큰 파일은 주기적으로 책임과 변경 이유를 점검한다. 둘 이상의 독립 책임이 자라났다면 안정된 응집 단위로 추출하되, 줄 수만 줄이기 위한 미세 파일·얇은 전달 계층·과분해는 만들지 않는다. 파일 크기는 검토 신호이지 위반 판정이 아니며, 분리는 소유권과 경계를 더 선명하게 만들 때만 한다.
14. **결정론적 동시성 검증:** 비동기 순서는 명시적 barrier나 테스트 훅으로 정확한 turn을 멈춘 뒤, 작업·frame을 의도한 순서로 투입하고 허용된 응답만 추월시킨다. 나머지 상대 순서와 최종 영속 snapshot을 함께 단언하며, sleep·임의 timeout·스케줄링 운을 정답 근거로 쓰지 않는다. timeout은 멈춘 테스트를 실패시키는 watchdog으로만 허용한다. 같은 operation의 재시도 멱등성과 동시 실행 결과도 검증한다.
15. **테스트 실행기의 증거성:** 테스트 환경은 제품 모듈이 설정을 읽기 전에 고정하고, 호스트 환경 변수와 실제 사용자 상태를 제거하며 임시 저장소를 사용한다. module-load 설정, 프로세스 전역 mock, DOM 유무, 실제 socket처럼 서로 오염되는 묶음은 별도 프로세스 partition으로 실행한다. 호환되지 않는 partition을 섞은 실행은 추측해 계속하지 말고 명령을 안내하며 거부한다. 루트·패키지 어느 경로에서 실행해도 필요한 setup과 사전 빌드를 적용하고 생성물·오래된 복제 테스트는 수집하지 않는다. 0개 테스트, 오래된 산출물, 잘못된 setup에서 나온 녹색은 통과 증거가 아니다.
16. **지원 표면 검증:** 사용자에게 보이는 동작은 공개 CLI와 실제 배포 형태를 통해 검증하고 내부 API 호출은 단위 검증의 보조로만 쓴다. 지원 하한보다 오래된 도구나 프로토콜은 거부하며, 알 수 없는 호환성을 추측해 구동하지 않는다.
