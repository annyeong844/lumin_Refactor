...
1. **SSOT와 불확실성:** 제품 계약 -> 시스템 청사진 -> 담당 아키텍처 문서 -> 활성 슬라이스 -> 이 문서 순서를 따른다. 계약이나 소유자가 불명확하면 수정 전에 멈추고 Workboard에서 소유 문서를 찾는다. 사용 가능한 경우 외부 `lumin_lab`을 보조 증거로 쓰되, 그 결과가 계약을 대체할 수 없다.
