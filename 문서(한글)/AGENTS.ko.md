...
11. **연대기 금지:** Workboard, 체크리스트, README, owner 문서를 완료 작업의 연대기로 만들지 않는다. 현재 계약·상태·다음 작업에 필요하지 않은 과거 구현 서술은 Git 이력에서 확인한다. 계약이 요구하는 provenance, 설계 결정, 독립 검토 기록만 지정된 owner에 보존한다.
12. **Rust에 code-map 금지:** 이 저장소의 code-map은 Rust 심볼을 지원하지 않는다. `.rs` 탐색·심볼 조회·수정 후 확인에 code-map을 호출하지 말고 `rg`와 직접 소스 읽기를 사용한다.
