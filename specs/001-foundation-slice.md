...
validates the transformed logical dump and complete namespace binding, removes and
flushes every intent-authorized private artifact before removing the intent, and reopens
v13 before success. On current v13 it validates and returns without replacing the store
...
admissible recovery artifact. The canonical intent self-binds its physical identity and
monotonic revision. Before any private source/target name becomes visible, a durable
`MigrationArtifactBinding` records its exact role, private name, generation, schema,
complete logical SHA-256, physical identity, and one-link state. Migration keeps the
intent durable while it opens each artifact no-follow, revalidates that the entry still
names the bound held object, disposes only that object through an identity-bound operating
system primitive, and flushes its absence; only then is the exact terminal intent removed
the same way as the final migration mutation. An unbound, substituted, multiply linked,
or no-intent artifact is foreign and survives the hard-stop. Concurrent migration
...
- `lumin store migrate` is the only private-v1 logical reader, uses transaction-scoped handles and generation fencing, publishes its intent without a crash-surviving pending name, physically binds each private artifact before visibility, retains that intent until entry-to-held-handle cleanup is durably complete, returns the same target-only `ready` response after first success or retry, and prevents an old-generation process from committing after replacement; ordinary repository-state commands route a live intent without reading its private payload and never migrate on open, while an unbound, substituted, multiply linked, or no-intent private artifact is foreign;
...
| `lifecycle-store-migration` | Public children prove that ordinary repository-state commands reject the exact v12 fixture and every matching live-intent fixture with the `lumin store migrate` instruction and no mutation, then the migration command alone advances or recovers them to ready v13. One process holds the v12 generation token while that command migrates; transaction-scoped handles close before replacement, state-directory/lock bindings remain fixed, the self/physical-binding intent survives until entry-to-held-handle artifact disposition and its parent flush complete, every crash point selects the ARCH-002 recovery rule, all logical records/references survive, and the old process must reopen/revalidate before its late mutation can commit. Exact deaths before the first intent byte, after a partial write, after candidate flush, after final-name publication, and before/during parent flush leave no staging name and only absent-or-complete canonical intent. A live-intent source with valid envelope/header but a corrupted private-v1 logical table first yields the ordinary migration-required diagnostic without any byte change, then makes `lumin store migrate` integrity-hard-stop with the canonical store, intent, source, and generation unchanged. Replacing either bound source or target with a logically identical different physical object, or adding a second link, hard-stops before disposition and preserves every object; a byte-identical valid v12 source introduced after intent removal is likewise foreign and never deleted. The fixture maps pending/interrupted cleanup rows to zero sequences, committed legacy `not-attempted` to unfinished synthetic sequence 1, and committed `succeeded`/`failed` to historical completed sequence 1 plus unfinished greatest sequence 2; every committed mapping initially projects `unknown`, and every invalid legacy shape fails before replacement. Initial success, recovery after replacement/private cleanup/output failure, and an already-current retry emit the same exact `lumin.lifecycle-store-migration.v1` bytes, while v13 retry performs no replacement or generation advance. Both packaged platform binaries execute their actual handle-owned publication and identity-bound disposition primitives, admission rejection, migration success, intent-held cleanup recovery, and byte-identical no-op retry rather than relying on the development CLI. |
...
31. `lumin store migrate` is the sole public v12-to-v13 route: ordinary repository-state commands return its exact fail-closed instruction for v12 or a matching live intent without reading private-v1 payloads, its target-only success DTO is byte-identical after first migration, crash/output recovery, and already-current retry, and v13 retry performs no replacement or generation advance. The command uses transaction-scoped handles and generation fencing, leaves no crash-surviving pending intent, self-binds the canonical intent, binds every private artifact's exact physical identity and one-link state before visibility, retains that authority until held-object disposition is durably flushed, and rejects every unbound/substituted/no-intent artifact as foreign. Every crash point has one recovery rule, an old-generation late writer cannot commit without reopening and revalidation, every committed private-v1 cleanup row maps to an unfinished synthetic greatest sequence and public `unknown`, and every invalid schema or intent-bound payload fails before replacement or deletion. Store-crash proves intent-publication boundaries, the header-valid/payload-corrupt responsibility split, physical substitution/multiple-link rejection, and foreign-copy rejection; both actual packaged platform binaries prove their publication/disposition primitives, admission, success, intent-held cleanup recovery, and retry.
...
| 31 | `lifecycle_migration_fences_generations` | `lifecycle-store-migration` | `lumin-xtask corpus foundation --store-crash`, `lumin-xtask package-check windows-x64`, and `lumin-xtask package-check linux-x64` | Store-crash public children drive multi-process v12-to-v13 fault injection, preserve one generation, reject/reopen every old-generation late mutation, prove every pre/during-publication death leaves no pending name and only absent-or-complete intent, retain physical intent authority through private cleanup, map exact v12 cleanup rows to canonical synthetic sequences or fail before replacement, prove ordinary diagnostic then migration hard-stop for a header-valid/payload-corrupt intent-bound source, reject bound source/target substitution and extra links before disposition, and reject a byte-identical no-intent source without deletion. Each actual packaged binary executes the platform handle-owned publication and identity-bound disposition primitives, rejects v12/live intent without mutation, successfully runs `lumin store migrate`, recovers replacement and intent-held cleanup boundaries, and returns the same target-only DTO on retry without another generation advance. |
