...
only v13 and never migrate on open. A valid v12 header or matching unfinished v12-to-v13
intent, whether the canonical file is v12 or v13, exits `1` with empty stdout and exactly
...
matching durable intent is foreign and every command hard-stops without reading,
adopting, or deleting it, even when it is a byte-identical copy of the old v12 store.
A never-initialized repository makes `lumin store migrate` exit `1` with empty stdout and
exactly `lumin: lifecycle store is not initialized\n` on stderr without creating `.lumin`
or changing any entry; a present but incomplete namespace remains an integrity hard-stop.
...
bound source/target identities, removes and flushes the retired source before removing
the intent, and reopens v13 before success. On current v13 it validates and returns
without exchanging the store
or advancing its generation.

Intent bytes are prepared and flushed on a same-volume handle-owned object whose final
disposition is armed before the first write. Only that held object may be published
no-replace as `lifecycle-migration.json`; death before publication removes it, while death
after publication but before the parent flush leaves either no intent or the complete
canonical intent. No `.pending`, partial canonical file, or other staging name is an
admissible recovery artifact. The canonical intent self-binds every revision and records
`MigrationArtifactBinding` values for the already opened canonical v12 source and held
v13 target, including each role, pre/post-exchange name, generation, schema, complete
logical SHA-256, physical identity, and one-link state. The target binding is durable
before its private name is visible.

Every intent publication/update/removal, target publication, source/target exchange, and
artifact disposition occurs under one operating-system `MigrationNamespaceIsolation`
that spans final identity/link-count validation, the mutation, entry/absence proof, and
parent flush. It excludes entry substitution and creation of another hard link to either
bound object until that interval ends; an advisory lock or later recheck is insufficient.
Migration uses an identity-preserving exchange/replace-with-backup primitive or an
isolated crash-recoverable sequence of no-replace moves, so the target becomes canonical
and the source becomes private without disposing either object, then keeps the intent durable
until the private source is disposed through its held handle under the same link-count
fence. A platform lacking isolation or identity-preserving exchange refuses before the
first mutation. An unbound, substituted, multiply linked, or no-intent object is foreign
and survives the hard-stop. Concurrent migration commands serialize; at most one advances
the generation and every follower validates v13 and returns the same response.
...
- `lumin store migrate` is the only private-v1 logical reader, refuses absent state without creating it, uses transaction-scoped handles and generation fencing, publishes its intent without a crash-surviving pending name, physically binds the canonical source, private target, and every intent revision, and exchanges source/target identities without pathname overwrite; it retains that intent until link-count-fenced held-object cleanup is durably complete, returns the same target-only `ready` response after first success or retry, and prevents an old-generation process from committing after exchange; ordinary repository-state commands route a live intent without reading its private payload and never migrate on open, while an unbound, substituted, multiply linked, or no-intent object is foreign;
...
| `lifecycle-store-migration` | Public children prove that ordinary repository-state commands reject the exact v12 fixture and every matching live-intent fixture with the `lumin store migrate` instruction and no mutation, then the migration command alone advances or recovers them to ready v13. A never-initialized fixture returns the exact not-initialized diagnostic and remains entry-for-entry unchanged without `.lumin`. One process holds the v12 generation token while migration runs; transaction-scoped handles close before exchange, state-directory/lock bindings remain fixed, and the self-bound intent records the canonical source plus private target physical identities and survives until link-count-fenced held-object disposition and parent flush complete. Exact deaths before the first intent byte, after a partial write, after candidate flush, after final-name publication, and before/during parent flush leave no staging name and only absent-or-complete canonical intent. A live-intent source with valid envelope/header but a corrupted private-v1 logical table first yields the ordinary migration-required diagnostic without any byte change, then makes migration integrity-hard-stop with every object unchanged. Barriers substitute canonical `lifecycle.store` and the private target before identity-preserving exchange, and substitute `lifecycle-migration.json` before a revision update and before terminal removal; each mismatch hard-stops before mutation and preserves original/substitute objects, intent, target, generation, and logical dump. Another barrier stops after final one-link validation while a competing child attempts a same-filesystem hard link; platform isolation prevents the link from committing through disposition and parent flush. A byte-identical valid v12 source introduced after intent removal is foreign and never deleted. The fixture maps pending/interrupted cleanup rows to zero sequences, committed legacy `not-attempted` to unfinished synthetic sequence 1, and committed `succeeded`/`failed` to historical completed sequence 1 plus unfinished greatest sequence 2; every committed mapping initially projects `unknown`, and every invalid legacy shape fails before exchange. Initial success, recovery after exchange/private cleanup/output failure, and an already-current retry emit the same exact `lumin.lifecycle-store-migration.v1` bytes, while v13 retry performs no exchange or generation advance. Both packaged platform binaries execute their actual handle-owned publication, namespace-isolation, identity-preserving exchange, link-count-fenced disposition, absent-store refusal, recovery, and retry. The packaged Codex and Claude adapter checks invoke only the public migration command after its diagnostic, validate its exact DTO, and retry the original public command without reading private state or embedding migration logic. |
...
31. `lumin store migrate` is the sole public v12-to-v13 route: ordinary repository-state commands return its exact fail-closed instruction for v12 or a matching live intent without reading private-v1 payloads, a never-initialized repository fails without creating state, its target-only success DTO is byte-identical after first migration, crash/output recovery, and already-current retry, and v13 retry performs no exchange or generation advance. The command uses transaction-scoped handles and generation fencing, leaves no crash-surviving pending intent, self-binds every intent revision, binds the canonical source and private target physical identities and one-link states, exchanges them through a no-disposition primitive or isolated crash-recoverable no-replace moves, and retains authority until link-count-fenced held-object disposition is durably flushed. Every crash point has one recovery rule; source, target, or intent substitution and a competing hard-link attempt cannot mutate or delete a foreign object; an old-generation late writer cannot commit without reopening and revalidation; every committed private-v1 cleanup row maps to an unfinished synthetic greatest sequence and public `unknown`; and every invalid schema or intent-bound payload fails before exchange or disposition. Store-crash proves all exact barriers, both actual packaged platforms prove their OS primitives and absent-state refusal, and both packaged skill adapters use only the public command and DTO.
...
| 31 | `lifecycle_migration_fences_generations` | `lifecycle-store-migration` | `lumin-xtask corpus foundation --store-crash`, `lumin-xtask package-check windows-x64`, `lumin-xtask package-check linux-x64`, and `lumin-xtask package-check skills` | Store-crash public children drive multi-process v12-to-v13 fault injection, preserve one generation, reject/reopen every old-generation late mutation, prove every pre/during-publication death leaves no pending name and only absent-or-complete intent, retain physical intent authority through private cleanup, map exact v12 cleanup rows to canonical synthetic sequences or fail before exchange, prove ordinary diagnostic then migration hard-stop for a header-valid/payload-corrupt intent-bound source, reject canonical-source/private-target and pre-update/pre-removal intent substitution, fence a hard-link attempt through disposition, reject a byte-identical no-intent source without deletion, and refuse absent-store initialization without changing any entry. Each actual packaged binary executes the platform handle-owned publication, namespace isolation, identity-preserving exchange, and link-count-fenced disposition primitives, rejects v12/live intent without mutation, successfully runs migration, covers absent state, recovers exchange/cleanup boundaries, and returns the same target-only DTO on retry without another generation advance. Both packaged adapters react to the recovery diagnostic by invoking the public migration command, accept only its exact DTO, and retry the original public command without embedded migration logic or private store access. |
