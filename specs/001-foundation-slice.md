...
| `capabilities-pagination` | Current-binary and exact-run capability collections expose the four grounded compiled rows as `3+1` pages under fixed `CAPABILITIES_PAGE_SIZE=3` and `capabilities.v1`, traversing every capability ID exactly once. The current-binary query needs no repository state, binds content-bound `lumin-binary-cursor.v2` to the model-owned semantic `BuildIdentity`, and rejects cross-build or run cursors; exact-run pagination reuses repository-bound `lumin-run-cursor.v2` and rejects cross-repository or cross-run reuse. |
