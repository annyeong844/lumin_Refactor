...
`lumin cache clean` is the ARCH-002 idempotent disposable mutation authorized by the ARCH-000 command set. It requires exactly one split-form `--operation-id <operation-id>` and accepts at most one split-form `--format json`; missing values, repeated or equals-form flags, other options, positional arguments, and non-JSON formats are malformed exit `2`. On success it exits `0`, writes one `lumin.cache-cleanup.v2` object whose only fields in canonical order are `schemaVersion`, `operationId`, `requestDigest`, and `status: "clean"`, writes the normal newline and nothing to stderr, preserves canonical run/gate/retention evidence and every namespace binding, and leaves the active cache anchor-only. Before any active payload inspection, the canonical cleanup operation record installs a mutation reservation that blocks every store-owned active-cache writer and different cleanup until result commit; the reservation survives every `pending` lease, process death, `interrupted` observation, and fresh-lease reattachment. Before any move, the same operation binds the initial quarantine authorization-set identity/count and every planned source, destination, physical-tree manifest, and digest in `Authorized` state; a syntactically valid self-hashed tree without exactly one matching authorization row is foreign state, and historical rows are not copied into each new operation. Cleanup flushes each complete payload tree bottom-up and remanifests before authorization, moves it without replacement, repeats the tree flush/remanifest, and durably flushes the cache, bound quarantine, and trash directories before marking the row `Validated`. Success commits one result only after all rows validate and the final namespace proof passes. Namespace, identity, topology, persistence, or delivery failure exits `1`; a pre-transport failure leaves stdout empty, `BrokenPipe` leaves stderr empty, and another stdout write/flush failure emits exactly `lumin: cannot write stdout\n` when stderr is writable. Every result transport first commits a strictly increasing delivery-attempt sequence and atomically projects `lastDeliveryStatus: "unknown"`, releases all guards and transactions, and later records completion. The projection is `not-attempted` only before any sequence allocation; it remains `unknown` whenever the greatest allocated sequence is unfinished, including when a lower sequence completes, and becomes `succeeded` or `failed` only from completion of the greatest allocated attempt. Process death or delivery failure recovers only with the same operation ID; `lumin operation show` emits one bounded read-only `lumin.cache-cleanup-operation.v2` projection and never performs liveness recovery. The amended binary provides no v1 negotiation or fallback: frozen v1 never contains `unknown`, and a v1-only consumer rejects the distinct v2 schema before reading the enum. Immediately after death it remains canonically `pending` with the continuous mutation reservation still held. An identical mutating retry under the exclusive guard proves the exact prior execution lease dead without a timeout, commits `interrupted` and one count increment without releasing that reservation, then releases the guard at an exact public-show barrier. A second guarded transaction attaches a fresh lease and returns to `pending` under the same reservation before reconciliation. Repeated shows and repeated recovery of the same interrupted attempt do not increment again; another ID cannot adopt pending state. Deterministic fixtures stop after delivery allocation and after partial and complete stdout transport but before completion. They force both completion orders: lower completion while the greater attempt is unfinished preserves `unknown` until the greater completion selects its result; greater completion while the lower attempt is held selects the result first, and the late lower completion must leave that status and the complete durable operation snapshot unchanged. They also cover foreign self-hashed quarantine, top-level and nested substitution, dirty trees, cache-writer rejection at exact barriers throughout process death and `pending -> interrupted -> pending`, the projection sequence, every unchanged rejected-writer snapshot, and death after authorization, rename visibility, physical durability, row validation, and final result commit. Cleanup never physically deletes quarantine entries.
...
| `lifecycle-operation-idempotency` | Pre/post, abandon, pin/unpin, prune-plan creation/confirmation, and cache cleanup use one operation contract: same ID/request joins, safely resumes a protocol-proven interruption, or returns one committed result; conflicting reuse is malformed. `operation show` is read-only and emits cleanup-operation v2; frozen v1 never carries `unknown`. After cleanup child death show returns `pending` until the same-ID mutating retry proves the exact lease dead, exposes one durable `interrupted` barrier/count increment, and installs a fresh `pending` attempt before resuming. Delivery barriers after sequence allocation and after partial or complete stdout before completion assert `unknown`. One ordered case completes the lower attempt while the greater remains unfinished and then completes the greater; the converse completes the greater while holding the lower and proves the lower late completion cannot change the selected status or durable snapshot. Recovery creates no duplicate cache authorization or move. |
...
17. Operation-ID retry and post-commit delivery recovery never duplicate any gate, retention, or cache-cleanup mutation; read-only show never changes liveness state, and interrupted cache movement is resumed only after the same-ID mutating retry records one exact dead-attempt transition and fresh execution lease. Exact barriers after allocation, after partial and complete stdout but before completion, and after an older completion while a greater attempt is unfinished all assert public `unknown`; completion of the greatest allocated attempt alone selects the final delivery status and operation snapshot.
