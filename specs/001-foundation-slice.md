...
the command validates either native no-journal state or the exact terminal journal/source
provenance, the target's immutable header/provenance facts, and its current-v13 integrity;
...
immutable authorization core, generations, and source/transformed-target user-logical
identities. The control row is excluded from the transformed user catalog but included in
the source's opaque bytes; only after that commit is the root finalized against the
...
target's terminal physical/header provenance and current logical integrity, not its
historical migration payload digest. Concurrent migration commands serialize; at most one
advances the generation and every follower validates v13 and returns the same response.
...
- `lumin store migrate` is the only private-v1 logical reader, refuses absent state without creating it, uses transaction-scoped handles and generation fencing, requires canonical-v12 authorization for revision zero, publishes a contiguous append-only journal without a crash-surviving pending name, and folds a closed pending/published/superseded/exchanged binding state machine before exchanging the physically bound source and target through a no-disposition platform primitive; it permanently retains the terminal journal and retired source rather than assuming an impossible Linux hard-link fence, admits later target mutation through immutable provenance plus current-v13 integrity rather than the migration-time payload hash, returns the same target-only `ready` response after first success or retry, and prevents an old-generation process from committing after exchange; ordinary repository-state commands route an unfinished journal without decoding private-v1 user records and never migrate on open, while migration alone admits the exact journal-proven canonical-absent intermediate and an unbound, substituted, or no-journal object is foreign;
