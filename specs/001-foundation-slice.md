...
`lumin cache clean` is the ARCH-002 idempotent disposable mutation. On success it exits `0`, writes exactly `{"schemaVersion":"lumin.cache-cleanup.v1","status":"clean"}` plus the normal newline to stdout, writes nothing to stderr, preserves every canonical record and the bound cache parent/anchor, and leaves the active cache parent with only the anchor. Validated payload trees are atomically moved without replacement into disjoint noncanonical cache-eviction entries below the bound trash parent; this command never physically deletes those quarantine entries. It accepts at most one split-form `--format json`; missing/unknown values, repeated format flags, equals-form flags, positional arguments, or a non-JSON format are malformed exit `2`. Namespace, payload-identity, topology, persistence, or result-delivery failure exits `1`; a pre-transport failure leaves stdout empty, while a stream failure may transfer only a nonauthoritative prefix. The command has no operation ID: a delivery failure may occur after noncanonical active-cache eviction and is recovered by rerunning the same idempotent command. Public deterministic barriers replace both a validated top-level name and a manifested nested child before the owning top-level move; each run must preserve the substitute in quarantine, preserve the immutable parents/anchors and required remaining order, publish no success, and match the final failed snapshot.
...
| `reserved-state-namespace` | `.lumin` symlink/junction/reparse/mount parents, nested redirected state parents, foreign/preexisting contents, `RepositoryId` mismatch, caller-declared state writes, physical aliases, and external state mutation each produce the exact malformed/integrity outcome without source evidence or gate success. Public cache cleanup follows the Section 6 command/response/exit/delivery contract, atomically evicts nested payload trees without physical deletion and remains idempotent, and deterministic top-level and nested-child validation-to-move substitutions hard-stop while preserving each substitute, the required remaining order and final failed snapshot, and the cache/trash parents and anchors. |
...
36. `.lumin` is a no-follow, repository-bound reserved namespace whose state-directory, immutable lock-object, namespace nonce, and exact four-kind managed-parent directory/anchor/parent-nonce set are marker/store-bound and revalidated; replacement/swap, aliases, redirected or copied parents, foreign contents, caller writes, external mutation, and validation-to-eviction substitution cannot enter source evidence or a successful mutation. Public cache cleanup obeys its exact JSON/exit/delivery contract, atomically detaches only identity- and tree-revalidated disposable payloads into disjoint trash quarantine, preserves the cache binding, and performs no pathname-based physical deletion.
...
| 36 | `state_namespace_is_reserved` | `reserved-state-namespace`, `state-namespace-initialization`, `state-lock-replacement-split-brain`, `state-managed-parent-replacement` | `lumin-xtask corpus foundation`, `lumin-xtask corpus foundation --determinism`, `lumin-xtask corpus foundation --store-crash`, plus `lumin-xtask package-check windows-x64`, `lumin-xtask package-check linux-x64`, and `lumin-xtask package-check skills` | Standard, determinism, and both packaged-platform runs prove cleanup's exact machine response/exits, rerun delivery recovery, anchor-only active-cache success, no physical deletion, and deterministic top-level/nested-child substitution failures with preserved order and final snapshots; the skill check proves the no-operation-ID adapter exception. Store-crash additionally proves the three crash-applicable state rows; it is not cleanup acceptance evidence. All lanes preserve one bound state domain and cannot authorize foreign state. |
