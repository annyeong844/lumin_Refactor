...
| `state-namespace-initialization` | Faults before/after `.lumin`, lock/global nonce, each top-level managed-parent/anchor identity and nonce, the nested cache-quarantine binding/anchor, marker temp/rename/flush, and store creation recover only the ARCH-002 absent, resumable exact complete binding, or foreign-state outcome. A marker/lock/store missing the nested binding returns `IncompatibleStateSchema` without lazy creation, adoption, or backend-only migration. |
...
17. Operation-ID retry and post-commit delivery recovery never duplicate any gate, retention, or cache-cleanup mutation; interrupted cache movement is adopted only by its matching canonical operation record.
...
| 17 | `lifecycle_mutations_are_idempotent` | `lifecycle-operation-idempotency` | `lumin-xtask corpus foundation` | Every gate, retention, or cache-cleanup mutation retry returns one committed result and `operation show` recovers injected delivery failure without a duplicate move or authorization row. |
