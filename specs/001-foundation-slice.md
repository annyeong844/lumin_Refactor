...
| `bounded-nested-query` | Run and gate-revision pages expose immutable scope, totals, truncation, and stable top-level and nested continuation. Engine is sole paging/validation/anchor owner with PAGE_SIZE=100, `lumin-run-cursor.v1`/`lumin-gate-cursor.v1` schemas, and `lumin explain --run <run-id> <finding-id>` for nested evidence/relations. |
...
| `capabilities-pagination` | Current-binary and exact-run capability collections expose the four grounded compiled rows as `3+1` pages under fixed `CAPABILITIES_PAGE_SIZE=3` and `capabilities.v1`, traversing every capability ID exactly once. The current-binary query needs no repository state, binds `lumin-binary-cursor.v1` to the model-owned semantic `BuildIdentity`, and rejects cross-build or run cursors; exact-run pagination reuses repository-bound `lumin-run-cursor.v1` and rejects cross-repository or cross-run reuse. |
...
| 12 | `all_pages_are_reachable` | `bounded-nested-query` | `lumin-xtask corpus foundation` | Run and gate-revision cursor traversal returns exactly `total` top-level and nested items without following a newer scope. A fixture with 1 library export and 101 test re-exports yields findings 102=100+2, evidence 102=100+2, relations 101=100+1 split at PAGE_SIZE=100. Cursors are bound to exact run_id (run scope) or gate_id+revision (gate scope). Cross-run, cross-gate, cross-revision, cross-finding, cross-collection (evidence vs relations), run-vs-gate, tampered, and cross-repository cursor reuse is rejected without page-1 restart. Immutable run cursors survive a second audit/repository mutation. `lumin-run-cursor.v1` and `lumin-gate-cursor.v1` schemas with `deny_unknown_fields` reject cross-scope deserialization. |
