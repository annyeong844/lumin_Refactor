...
`lumin cache clean` is the ARCH-002 idempotent disposable mutation. On success it exits `0`, writes exactly `{"schemaVersion":"lumin.cache-cleanup.v1","status":"clean"}` plus the normal newline to stdout, writes nothing to stderr, preserves every canonical record and the bound cache parent/anchor, and leaves the active cache parent with only the anchor. Before inspecting active payloads, every invocation admits the complete pre-existing cache quarantine, even when the active cache is empty; malformed names, redirects, mount crossings, unsupported entries, multiply linked regular files, or topology disagreement are integrity failures. Validated payload trees are atomically moved without replacement into disjoint noncanonical cache-eviction entries below the bound trash parent; creating that quarantine is durably flushed in its trash parent before the first move, and this command never physically deletes quarantine entries. It accepts at most one split-form `--format json`; missing/unknown values, repeated format flags, equals-form flags, positional arguments, or a non-JSON format are malformed exit `2`. Namespace, payload-identity, topology, persistence, or result-delivery failure exits `1`; a pre-transport failure leaves stdout empty. A stdout failure after a nonauthoritative prefix exits `1`: `BrokenPipe` leaves stderr empty, while another write/flush failure emits exactly `lumin: cannot write stdout\n` when stderr is writable. The command has no operation ID: process death or delivery failure after noncanonical active-cache eviction is recovered by rerunning the same idempotent command. Public deterministic barriers replace both a validated top-level name and a manifested nested child before the owning top-level move; each run must preserve the substitute in quarantine, preserve the immutable parents/anchors and required remaining order, publish no success, and match the final failed snapshot. A separate public child-process fixture terminates cleanup after the first durable move and proves that restart preserves prior quarantine and evicts only the remaining active set.
...
| `reserved-state-namespace` | `.lumin` symlink/junction/reparse/mount parents, nested redirected state parents, foreign/preexisting contents, `RepositoryId` mismatch, caller-declared state writes, physical aliases, and external state mutation each produce the exact malformed/integrity outcome without source evidence or gate success. Public cache cleanup follows the Section 6 command/response/exit/delivery contract, admits valid prior quarantine even on an empty cache, rejects malformed prior quarantine, durably publishes a newly created quarantine, atomically evicts nested payload trees without physical deletion, and remains idempotent. Deterministic top-level and nested-child validation-to-move substitutions hard-stop while preserving each substitute, the required remaining order and final failed snapshot, and the cache/trash parents and anchors; a deterministic public process death after the first durable move restarts into the valid combined quarantine and anchor-only active cache. |
...
36. `.lumin` is a no-follow, repository-bound reserved namespace whose state-directory, immutable lock-object, namespace nonce, and exact four-kind managed-parent directory/anchor/parent-nonce set are marker/store-bound and revalidated; replacement/swap, aliases, redirected or copied parents, foreign contents, caller writes, external mutation, invalid pre-existing cache quarantine, and validation-to-eviction substitution cannot enter source evidence or a successful mutation. Public cache cleanup obeys its exact JSON/exit/stderr/delivery contract, durably publishes any new quarantine, atomically detaches only identity- and tree-revalidated disposable payloads into disjoint trash quarantine, recovers a partial process-death eviction by whole-command rerun, preserves the cache binding, and performs no pathname-based physical deletion.
...
| 36 | `state_namespace_is_reserved` | `reserved-state-namespace`, `state-namespace-initialization`, `state-lock-replacement-split-brain`, `state-managed-parent-replacement` | `lumin-xtask corpus foundation`, `lumin-xtask corpus foundation --determinism`, `lumin-xtask corpus foundation --store-crash`, plus `lumin-xtask package-check windows-x64`, `lumin-xtask package-check linux-x64`, and `lumin-xtask package-check skills` | Standard and determinism run the public child-process restart after the first durable move, both substitution barriers, exact partial-stream exit/stderr cases, valid empty-cache quarantine, malformed-quarantine rejection, and anchor-only final snapshots. Both packaged-platform runs prove the command grammar/response/exits, durable first quarantine creation, retry, and no physical deletion through the distributed CLI; the skill check proves the no-operation-ID adapter exception. Store-crash proves only the three crash-applicable state rows and is not cleanup acceptance evidence. All lanes preserve one bound state domain and cannot authorize foreign state. |
