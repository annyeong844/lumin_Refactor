...
Status: active implementation; REVIEW-004 follow-up remains blocked pending freeze
...
| `.lumin`, marker-bound `lifecycle.lock`, any top-level managed or nested cache-quarantine parent, or any immutable anchor is replaced, multiply linked, a symlink/junction/reparse/mount crossing, has foreign contents/schema, or disagrees with the marker/store-bound repository/root/state-directory/lock/global-nonce/exact-binding set | `ForeignStateNamespace` or store-integrity hard-stop; no scan result, pointer, cleanup success, or gate revision |
...
| Root-absolute internal-looking specifier, package `imports`, or unsupported alias | Return typed `Unsupported` or `Unresolved` evidence with the limitation scope below; never skip the record. An unresolved outcome distinguishes explicit probe targets, a conclusively closed workspace-package surface with no target, and opaque workspace target derivation. URL imports are complete external/non-source outcomes and do not create an internal-consumer limitation. |
...
| Reason variant | Fact owner | Scope and target derivation | Downstream absence effect | Gate relevance before lifecycle delta | Static policy (`Scope/Absence/Gate`) |
| --- | --- | --- | --- | --- | --- |
| `JsRecoverableParseLocal` | `lumin-js` | `File`; module-use extraction is proven complete. | Keep extracted uses; disable unsupported local-definition absence only. | Required evidence gap; emit `RequiredEvidenceIncomplete` only when the gate needs the missing local fact. | `File/LocalDefinitions/RequiredEvidence` |
| `JsModuleUseUnknown` | `lumin-js` | `Workspace`; the file may hide a consumer anywhere in scan scope. | Disable intersecting workspace absence claims. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `Workspace/WorkspaceConsumers/RequiredEvidence` |
| `SourcePayloadUnavailable` | `lumin-inventory` | `Workspace` for an admitted unreadable source. | Disable workspace absence because its imports cannot be bounded. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `Workspace/WorkspaceConsumers/RequiredEvidence` |
| `DynamicImportNonLiteral` | `lumin-js` | `ExplicitTargets` only for a finite derived target set; `SourceInventory` compactly denotes every current source for a valid relative static prefix whose arbitrary suffix may cross lexical directories; otherwise `Workspace`. `SourceInventory` carries no repeated candidate vector. | Treat `ExplicitTargets` and `SourceInventory` as broad value consumers while preserving production/test fan-in; `Workspace` disables workspace absence. | A finite `ExplicitTargets` opacity fact enters lifecycle delta classification. `SourceInventory` remains `RequiredEvidenceIncomplete` because this slice has no durable reservation for future source-inventory membership; it cannot authorize an active gate. `Workspace` remains required incomplete evidence. | `ExplicitTargetsOrSourceInventoryOrWorkspace/CandidateConsumers/NormalizedOpacityOrRequiredEvidence` |
| `ImportMetaGlobUnsupported` | `lumin-js` | `ExplicitTargets` from a literal static base, otherwise the importer's `Package`. | Disable absence only in the derived target/package domain. | Normalized opacity fact enters lifecycle delta classification; an unbounded required target remains `RequiredEvidenceIncomplete`. | `ImportedTargetsOrPackage/CandidateConsumers/NormalizedOpacityOrRequiredEvidence` |
| `CommonJsComputedMember` | `lumin-js` | `Module` for the resolved target. | Mark all value exports on that module broadly consumed, without exact fan-in. | Normalized opacity fact enters lifecycle delta classification. | `ResolvedModule/ModuleValueExports/NormalizedOpacity` |
| `VueTemplateOpaque` | `lumin-sfc` | Imported component candidates and observed global registrations as `ExplicitTargets`; otherwise the parent `Package`. | Disable component-identity absence in that domain. | Normalized opacity fact enters lifecycle delta classification; unbounded required binding remains `RequiredEvidenceIncomplete`. | `ImportedTargetsOrPackage/ComponentIdentity/NormalizedOpacityOrRequiredEvidence` |
| `SfcDecompositionUnknown` | `lumin-sfc` | `Workspace`; script/resource boundaries or module-use completeness are unknown. | Disable workspace consumer absence. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `Workspace/WorkspaceConsumers/RequiredEvidence` |
| `VueExternalScriptModeConflict` | `lumin-sfc` | Parent and external source owner `Package`; `Workspace` when their owners differ or cannot be proven. | Disable script-consumer and template-binding absence in that domain. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `ParentAndTargetOwnersOrWorkspace/ScriptAndTemplateBindings/RequiredEvidence` |
| `SfcDialectUnavailable` | `lumin-sfc` | `Workspace`. | No dead-consumer absence may rely on the unparsed dialect. | Required owner gap; emit `RequiredOwnerUnavailable` when the gate requires that dialect. | `Workspace/WorkspaceConsumers/RequiredOwner` |
| `InternalSpecifierUnresolved` | `lumin-resolve` | Ordered probe candidates as `ExplicitTargets`; a conclusively unmatched or null workspace-package export as `KnownNoTarget`; `Workspace` only when target derivation is opaque. | Disable absence for candidates; a known empty target set does not disable unrelated absence; opaque configuration disables workspace absence. | Complete normalized unresolved fact, including a known empty target set, enters lifecycle delta classification; opaque target derivation remains `RequiredEvidenceIncomplete`. | `ExplicitTargetsOrKnownNoTargetOrWorkspace/CandidateConsumers/NormalizedUnresolvedOrRequiredEvidence` |
| `PackageImportsUnsupported` | `lumin-resolve` | The importer's `Package`. | Disable package-local absence because `#` mappings may hide internal consumers. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `OwningPackage/PackageConsumers/RequiredEvidence` |
| `AliasShapeUnsupported` | `lumin-resolve` | `Package` when every affected importer has one owner, otherwise `Workspace`. | Disable absence in the affected configuration domain. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `SourceOwnerPackageOrWorkspace/PackageConsumers/RequiredEvidence` |
| `AbsoluteInternalSpecifierUnsupported` | `lumin-resolve` | `Workspace`. | Disable workspace absence; the target may be any root-contained source. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `Workspace/WorkspaceConsumers/RequiredEvidence` |
| `TsconfigPayloadUnavailable` | `lumin-inventory` | `Package` when all importers controlled by the unreadable input share one owner, otherwise `Workspace`. | Disable absence in that configuration domain. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `ConfiguredPackagesOrWorkspace/ConfigurationDomain/RequiredEvidence` |
...
| `ImporterFormatUnsupported` | `lumin-resolve` | The owning `Package`; `Workspace` when package ownership is unavailable. | Disable Node16/NodeNext value-target and consumer absence because import/require lane selection is unknown. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `OwningPackageOrWorkspace/PackageTargetsAndConsumers/RequiredEvidence` |
| `PublicSurfaceUnsupported` | `lumin-resolve` | The owning `Package`, including `typesVersions`, unsupported `exports`, or another registered package public-surface shape. | Do not protect every sibling and do not emit package-surface absence. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `OwningPackage/PublicSurface/RequiredEvidence` |
| `GeneratedVirtualUnknown` | `lumin-resolve` | Observed generated-map targets as `ExplicitTargets`; otherwise the importer's `Package`. | Disable absence in the derived domain. | Normalized opacity fact enters lifecycle delta classification; unbounded required targets remain `RequiredEvidenceIncomplete`. | `ImportedTargetsOrPackage/CandidateConsumers/NormalizedOpacityOrRequiredEvidence` |
| `ScanOrIgnoreInputUnobservable` | `lumin-inventory` | `Workspace`. | Disable workspace absence because scan membership is unknown. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `Workspace/WorkspaceConsumers/RequiredEvidence` |
| `PackageMetadataUnobservable` | `lumin-inventory` | The known owner `Package`, otherwise `Workspace`. | Disable owner/public/dependency absence in that domain. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `ManifestOwnerPackageOrWorkspace/PublicSurface/RequiredEvidence` |
| `PackageIdentityUnsupported` | `lumin-inventory` | `Workspace`; package-name lookup and cross-package ownership cannot be bounded to an invalid/empty identity or duplicate workspace names. | Disable package-name, workspace bare-specifier, and dependency-owner absence; never select a duplicate winner. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `Workspace/PackageIdentityAndDependencyOwner/RequiredEvidence` |
| `PackagePrivacyUnsupported` | `lumin-inventory` | The owning `Package`. | Disable external-public and private-package absence policy for that package. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `OwningPackage/PackagePrivacy/RequiredEvidence` |
| `DependencyOwnerAmbiguous` | `lumin-inventory` | The source owner's `Package`; `Workspace` when package ownership is also ambiguous. | Disable dependency-owner absence and inferred lockfile writes in that domain. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `OwningPackageOrWorkspace/DependencyOwnerAndInferredWrites/RequiredEvidence` |
| `WorkspaceOwnershipUnsupported` | `lumin-inventory` | `Workspace`. | Disable workspace/package ownership absence. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `Workspace/WorkspaceOwnership/RequiredEvidence` |
| `PnpmDependencySemanticsUnsupported` | `lumin-inventory` | The pnpm workspace `Workspace`. | Disable dependency-owner and inferred manifest/lockfile-write absence while unsupported catalog/package-config semantics are present. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `WorkspaceFromConfig/PnpmDependencyAndInferredWrites/RequiredEvidence` |
| `ExplicitEntryUnavailable` | `lumin-inventory` | The derivable owner `Package`, otherwise `Workspace`. | Disable unreachable-module absence for that domain. | Required evidence gap; emit `RequiredEvidenceIncomplete` when intersecting required gate evidence. | `EntryOwnerPackageOrWorkspace/UnreachableModules/RequiredEvidence` |
| `CapabilityUnavailable` | `lumin-engine` capability registry | Declared paths/analysis area as `ExplicitTargets`; SFC dialects use the stricter row above. | Disable unavailable language, shape, clone, or discipline claims without rerouting ownership. | Required owner gap; emit `RequiredOwnerUnavailable` when the gate requires that capability. | `ExplicitTargetsOrWorkspace/CapabilityClaims/RequiredOwner` |
...
`lumin cache clean` is the ARCH-002 idempotent disposable mutation authorized by the
ARCH-000 command set. It requires exactly one split-form `--operation-id <operation-id>`
and accepts at most one split-form `--format json`; missing values, repeated or
equals-form flags, other options, positional arguments, and non-JSON formats are
malformed exit `2`. On success it exits `0`, writes one `lumin.cache-cleanup.v2` object
whose only fields in canonical order are `schemaVersion`, `operationId`, `requestDigest`,
and `status: "clean"`, writes the normal newline and nothing to stderr, preserves canonical
run/gate/retention evidence and every namespace binding, and leaves the active cache
anchor-only. Before any active payload inspection, the canonical cleanup operation record
installs a mutation reservation that blocks every store-owned active-cache writer and
different cleanup until result commit; the reservation survives every `pending` lease,
process death, `interrupted` observation, and fresh-lease reattachment. Before any move,
the same operation binds the initial quarantine authorization-set identity/count and every
planned source, destination, physical-tree manifest, and digest in `Authorized` state; a
syntactically valid self-hashed tree without exactly one matching authorization row is
foreign state, and historical rows are not copied into each new operation. Cleanup
flushes each complete payload tree bottom-up and remanifests before authorization, moves
it without replacement, repeats the tree flush/remanifest, and durably flushes the cache,
bound quarantine, and trash directories before marking the row `Validated`. Success
commits one result only after all rows validate and the final namespace proof passes.
Namespace, identity, topology, persistence, or delivery failure exits `1`; a
pre-transport failure leaves stdout empty, `BrokenPipe` leaves stderr empty, and another
stdout write/flush failure emits exactly `lumin: cannot write stdout\n` when stderr is
writable. Every result transport first commits a strictly increasing delivery-attempt
sequence and atomically projects `lastDeliveryStatus: "unknown"`, releases all guards and
transactions, and later records completion. The projection is `not-attempted` only before
any sequence allocation; it remains `unknown` whenever the greatest allocated sequence is
unfinished, including when a lower sequence completes, and becomes `succeeded` or
`failed` only from completion of the greatest allocated attempt. Process death or
delivery failure recovers only with the same operation ID; `lumin operation show` emits
one bounded read-only `lumin.cache-cleanup-operation.v2` projection and never performs
liveness recovery. The amended binary provides no v1 negotiation or fallback: frozen
public v1 never contains `unknown`, and a v1-only consumer rejects the distinct v2 schema
before reading the enum. Explicit lifecycle migration advances the private operation
record to `lumin-cache-cleanup-operation.v2`: a valid pending/interrupted private-v1 row
retains zero delivery sequences, committed legacy `not-attempted` becomes synthetic
allocated sequence 1 without completion, and committed legacy `succeeded`/`failed`
retains that result as completed historical sequence 1 while receiving an unfinished
greatest allocation at sequence 2. Every migrated committed row therefore projects
public `unknown`: private v1 cannot prove that no newer retry was already transporting
after release of all store and liveness locks. Any other legacy shape is
`IncompatibleStateSchema` before replacement; no query performs lazy conversion, and
the next current delivery allocates sequence 2 or 3 respectively. Only completion of
that current greatest attempt may replace the migrated `unknown` projection.
Immediately after death the operation remains canonically `pending` with the continuous
mutation reservation still held. An identical mutating retry under the exclusive guard
proves the exact prior execution lease dead without a timeout, commits `interrupted` and
one count increment without releasing that reservation, then releases the guard at an
exact public-show barrier. A second guarded transaction attaches a fresh lease and returns
to `pending` under the same reservation before reconciliation. Repeated shows and repeated
recovery of the same interrupted attempt do not increment again; another ID cannot adopt
pending state. Deterministic fixtures stop after delivery allocation and after partial and
complete stdout transport but before completion. They force both completion orders:
lower completion while the greater attempt is unfinished preserves `unknown` until the
greater completion selects its result; greater completion while the lower attempt is held
selects the result first, and the late lower completion leaves that public v2 projection
byte-for-byte unchanged while the private ledger gains exactly the lower sequence/result
without changing either greatest sequence or any other field. They also cover foreign
self-hashed quarantine, top-level and nested substitution, dirty trees, cache-writer
rejection at exact barriers throughout process death and
`pending -> interrupted -> pending`, the projection sequence, every unchanged
rejected-writer snapshot, and death after authorization, rename visibility, physical
durability, row validation, and final result commit. Cleanup never physically deletes
quarantine entries.
...
journal, or exact journal-proven canonical-absent exchange intermediate exits `1` with
...
hard-stop without a guessed migration route. Any private source or target without that
matching durable journal is foreign and every command hard-stops without adopting or
deleting it, even when it is a byte-identical copy of the old v12 store. A never-initialized
repository makes `lumin store migrate` exit `1` with empty stdout and exactly
`lumin: lifecycle store is not initialized\n` on stderr without creating `.lumin` or
changing any entry. A present but incomplete namespace is an integrity hard-stop except
when migration alone proves the exact source-retired/canonical-absent placement from the
maximal valid unfinished journal and both bound objects.
...
v12-to-v13 journal; it neither initializes absent state nor chains, downgrades, or guesses
other schemas. On v12 it exclusively performs or recovers the generation-fenced copy,
validates the transformed logical dump and complete namespace binding, exchanges the
bound source/target identities without disposing either, appends a terminal journal
revision, and reopens v13 before success. The retired source and journal remain immutable
reserved provenance. The migration-time target payload is authenticated through
terminalization and then becomes the ordinary mutable canonical v13 store. On current v13
the command validates either native no-journal/no-anchor state or the exact terminal
journal/source provenance and matching migrated-target header anchor, plus current-v13 integrity;
it does not compare legitimate later store contents with the migration-time payload hash
and returns without exchanging the store or advancing its generation.

Revision zero is prepared on a same-volume handle-owned object whose unpublished lifetime
is automatically disposed. Before publication, canonical v12 commits the greatest
append-only `MigrationRootAuthorization` that binds that candidate's physical identity,
immutable authorization core, generations, and source user-logical identity. The selected
authorization sequence/core and source identity form the target header's immutable
`MigrationProvenanceAnchor`; the exact transformed-target user-logical identity including
that anchor is then bound by the authorization row. The authorization core excludes that
target digest and the later post-authorization source bytes, so the framing is acyclic. The
control row is excluded from the transformed user catalog but included in the source's
opaque bytes; only after that commit is the root finalized against the
post-authorization source, flushed, and published no-replace as
`lifecycle-migration.json`. A self-consistent root without the exact greatest authorization
is foreign. Death before publication removes the candidate; recovery with no visible root
creates a fresh candidate and greater authorization rather than reusing the dead identity.
Every successor is another self-bound immutable object published no-replace at
`lifecycle-migration.revision-<sequence>.json` and bound to its exact predecessor name,
physical identity, and payload digest. No predecessor is replaced or removed. Death at
...
`SupersededUnpublished`, followed by a fresh binding. Published targets can never be
superseded, and only current published/terminal objects—not superseded absent identities—
are required during recovery validation.

Migration uses Linux `renameat2(RENAME_EXCHANGE)` or Windows handle-relative
`SetFileInformationByHandle(FileRenameInfo, ReplaceIfExists = false)` moves. Those
primitives never physically dispose
a published artifact; the target becomes canonical and the source remains at its bound
retirement name. Handle-relative identity/link-count checks run before and after every
turn, but the contract does not invent an unprivileged Linux authority that blocks a
noncooperating same-UID `linkat`. A racing extra link or identity mismatch prevents
terminalization or fails the next admission while retaining the source, target, complete
journal, and foreign link. An unbound, substituted, or no-journal object is foreign and
survives the hard-stop. After terminalization, current-v13 admission authenticates the
target's terminal physical identity, exact immutable root/source header anchor, and current
logical integrity, not its historical migration payload digest. Concurrent migration
commands serialize; at most one advances the generation and every follower validates v13
...
initial success, post-exchange recovery, and already-current retry are byte-identical.
It has no operation ID or operation-show variant: the single target schema and durable
append-only journal make it intrinsically idempotent, and it performs no new gate,
...
- caller-retained operation IDs make every gate, retention, and cache-cleanup mutation idempotent and recoverable through `lumin operation show`;
...
- `lumin store migrate` is the only private-v1 logical reader, refuses absent state without creating it, uses transaction-scoped handles and generation fencing, requires canonical-v12 authorization for revision zero and the migrated target's immutable root/source header anchor, publishes a contiguous append-only journal without a crash-surviving pending name, and folds a closed pending/published/superseded/exchanged binding state machine before exchanging the physically bound source and target through a no-disposition platform primitive; it permanently retains the terminal journal and retired source rather than assuming an impossible Linux hard-link fence, admits later target mutation through that immutable anchor plus current-v13 integrity rather than the migration-time payload hash, returns the same target-only `ready` response after first success or retry, and prevents an old-generation process from committing after exchange; ordinary repository-state commands route an unfinished journal without decoding private-v1 user records and never migrate on open, while migration alone admits the exact journal-proven canonical-absent intermediate and an unbound, substituted, or no-journal object is foreign;
...
| `workspace-package-exports` | Exact/pattern exports and edge-specific conditions resolve deterministically and define identity-scoped public surfaces; unmatched and null exports retain a typed known-empty target scope without suppressing unrelated dead findings. |
...
| `reserved-state-namespace` | `.lumin` symlink/junction/reparse/mount parents, nested redirected state parents, foreign/preexisting contents, `RepositoryId` mismatch, caller-declared state writes, physical aliases, and external state mutation each produce the exact malformed/integrity outcome without source evidence or gate success. Public cache cleanup follows the Section 6 command/result/delivery contract, accepts quarantine only through exact store-owned authorization rows, rejects well-formed but unauthorized trees, flushes and remanifests dirty regular/nested payloads before authorization and after movement, durably flushes the cache/quarantine/trash parent chain, and performs no physical deletion. Exact barriers cover top-level and nested substitution, death after authorization, rename visibility, physical flush, row validation, result commit, both delivery-completion orders, and one competing active-cache writer at process-death plus each `pending -> interrupted -> pending` boundary. A late lower completion leaves the public projection byte-identical and appends only its private evidence. Every competing write is rejected before mutation; recovery requires the same operation ID and preserves the exact remaining order, authorization states, bound parents/anchors, active-cache bytes, and final snapshot. |
| `state-namespace-initialization` | Faults before/after `.lumin`, lock/global nonce, each top-level managed-parent/anchor identity and nonce, the nested cache-quarantine binding/anchor, marker temp/rename/flush, and store creation recover only the ARCH-002 absent, resumable exact complete binding, or foreign-state outcome. The lock header carries only global bootstrap fields; a marker or store missing the nested binding returns `IncompatibleStateSchema` without lazy creation, adoption, or backend-only migration. |
...
| `state-managed-parent-replacement` | Public child-process faults swap `runs`, `trash`, or nested `trash/cache-evictions` with a copied or empty real same-volume directory before admission, after complete binding validation, immediately before run rename/trash/cache move, and before final guarded commit; attempts/cache, every anchor replacement/extra-link, hard-linked children, and cross-volume controls are included. Marker/store-bound top-level and nested directory/anchor/nonce tuples admit only the original binding set; no pointer, `Pruned`, cleanup success, or gate revision becomes canonical in either domain. |
...
| `lifecycle-operation-idempotency` | Pre/post, abandon, pin/unpin, prune-plan creation/confirmation, and cache cleanup use one operation contract: same ID/request joins, safely resumes a protocol-proven interruption, or returns one committed result; conflicting reuse is malformed. `operation show` is read-only and emits cleanup-operation v2; frozen public v1 never carries `unknown`. After cleanup child death show returns `pending` until the same-ID mutating retry proves the exact lease dead, exposes one durable `interrupted` barrier/count increment, and installs a fresh `pending` attempt before resuming. Delivery barriers after sequence allocation and after partial or complete stdout before completion assert `unknown`. One ordered case completes the lower attempt while the greater remains unfinished and then completes the greater; the converse completes the greater while holding the lower and proves the lower late completion cannot change the byte-identical public projection while the private ledger gains only that lower result. Recovery creates no duplicate cache authorization or move. |
...
| `bounded-nested-query` | Run and gate-revision pages expose immutable scope, totals, truncation, and stable top-level and nested continuation. Engine is sole paging/validation/anchor owner with PAGE_SIZE=100 and canonical nonterminal resume boundaries; content-bound `lumin-run-cursor.v2`/`lumin-gate-cursor.v2` schemas reject structured payload mutation, and `lumin explain --run <run-id> <finding-id>` owns nested evidence/relations. |
...
| `capabilities-pagination` | Current-binary and exact-run capability collections expose the five grounded compiled rows—including `inventory/dependency-ownership.v1`—as `3+2` pages under fixed `CAPABILITIES_PAGE_SIZE=3` and `capabilities.v1`, traversing every capability ID exactly once. The current-binary query needs no repository state, binds content-bound `lumin-binary-cursor.v2` to the model-owned semantic `BuildIdentity`, and rejects cross-build or run cursors; exact-run pagination reuses repository-bound `lumin-run-cursor.v2` and rejects cross-repository or cross-run reuse. |
...
| `cache-cleanup-publication-race` | Exact barriers force cleanup, publication, retention confirmation, and migration to contend for the same marker-bound exclusive guard; only one guarded interval advances at a time, no pointer or plan regresses, and no authorization row or cache move duplicates. |
...
| `retention-plan-pagination` | A prepared plan allocates one repository-scoped ID/content identity; content-bound `lumin-retention-cursor.v3` binds the fixed 100-record page policy and canonical nonterminal resume boundary, unrelated repository mutation does not break its cursor, and cross-plan or nonboundary cursor reuse is malformed. |
...
| `lifecycle-store-migration` | Public children prove that ordinary repository-state commands reject the exact v12 fixture, every matching unfinished-journal fixture, and the exact journal-proven canonical-absent intermediate with the `lumin store migrate` instruction and no mutation; migration alone advances or recovers them to ready v13. A never-initialized fixture returns the exact not-initialized diagnostic and remains entry-for-entry unchanged without `.lumin`, while an unexplained missing canonical store hard-stops. One process holds the v12 generation token while migration runs; transaction-scoped handles close before exchange and state-directory/lock bindings remain fixed. Root barriers stop before/after canonical-v12 authorization and before/after revision-zero publication: a self-consistent root lacking the greatest authorization is foreign, and a dead unpublished candidate is replaced only after a greater authorization makes the old identity permanently stale. Successor barriers stop before the first byte, after partial write, after candidate flush, after unique final-name publication, after reopen, and before/during parent flush; restart sees exactly the prior contiguous immutable chain or that chain plus one complete successor, never a staging name, replaced predecessor, gap, or fork. Target fixtures stop with `PendingPublication` both before and after target visibility: an absent proven-dead binding becomes exactly `SupersededUnpublished` plus one fresh pending identity whose old name must remain absent, while a visible exact target is authenticated and followed only by `Published`. A journal-bound source with valid envelope/header but a corrupted private-v1 logical table first yields the ordinary migration-required diagnostic without any byte change, then makes migration hard-stop with every object unchanged. Barriers substitute canonical `lifecycle.store`, the private target, revision zero, and the current predecessor before their next operation; every mismatch preserves all objects and logical records. A competing child creates a real same-filesystem hard link after the pre-exchange one-link check while migration is held; after the link becomes visible, Linux's actual `renameat2(RENAME_EXCHANGE)` and Windows's actual handle-relative `SetFileInformationByHandle(FileRenameInfo, ReplaceIfExists = false)` path never dispose a published object, reject terminal success on the extra link, and retain the journal, retired source, target, and foreign link. A byte-identical private source without a matching journal is foreign and never deleted. The fixture maps pending/interrupted cleanup rows to zero sequences, committed legacy `not-attempted` to unfinished synthetic sequence 1, and committed `succeeded`/`failed` to historical completed sequence 1 plus unfinished greatest sequence 2; every committed mapping initially projects `unknown`, and every invalid legacy shape fails before exchange. After terminal success, changing or cross-binding only the immutable target-header `MigrationProvenanceAnchor` hard-stops fresh admission; an ordinary v13 lifecycle mutation instead keeps that anchor fixed while changing the canonical target bytes/logical dump, after which a fresh ordinary command and already-current migration retry validate current-v13 integrity without comparing the historical target digest and the terminal journal/retired source remain byte-identical. Initial success, recovery after exchange/terminal-publication/output failure, and an already-current retry emit the same exact `lumin.lifecycle-store-migration.v1` bytes, while v13 retry performs no exchange, journal append, deletion, or generation advance. Both packaged platform binaries execute their real handle-owned append-only publication and no-disposition exchange primitives, root authorization, typed target recovery, canonical-absent recovery, absent-store refusal, anchor validation, post-migration mutation admission, and retry. The packaged Codex and Claude adapter checks invoke only the public migration command after its diagnostic, validate its exact DTO, and retry the original public command without reading private state or embedding migration logic. |
...
17. Operation-ID retry and post-commit delivery recovery never duplicate any gate, retention, or cache-cleanup mutation; read-only show never changes liveness state, and interrupted cache movement is resumed only after the same-ID mutating retry records one exact dead-attempt transition and fresh execution lease. Exact barriers after allocation and after partial and complete stdout before completion assert public `unknown`. Lower-first completion remains unknown until the greater result commits; greater-first completion fixes the public v2 projection before the held lower completion appends only its private sequence/result evidence.
...
31. `lumin store migrate` is the sole public v12-to-v13 route: ordinary repository-state commands return its exact fail-closed instruction for v12, a matching unfinished journal, or the exact journal-proven canonical-absent intermediate without decoding private-v1 user records; a never-initialized repository fails without creating state. Its target-only success DTO is byte-identical after first migration, crash/output recovery, and already-current retry, and v13 retry performs no exchange, journal append, deletion, or generation advance. The command uses transaction-scoped handles and generation fencing, leaves no crash-surviving pending name, requires the greatest canonical-v12 root authorization and matching immutable target-header `MigrationProvenanceAnchor`, publishes each intent successor at a unique append-only name without replacing its predecessor, and folds the exact observed/pending/published/superseded/exchanged/terminal binding state before exchanging source and target through Linux `renameat2(RENAME_EXCHANGE)` or Windows handle-relative `SetFileInformationByHandle(FileRenameInfo, ReplaceIfExists = false)` moves. The terminal journal and retired source remain immutable provenance because supported Linux cannot fence a noncooperating same-UID hard-link creation through disposition; the canonical v13 target is mutable after terminalization and later admission checks its immutable root/source anchor plus current-v13 integrity rather than the historical target payload hash. Every crash point has one recovery rule; source, target, or journal substitution and a competing hard-link attempt cannot make Lumin delete a foreign object; an old-generation late writer cannot commit without reopening and revalidation; every committed private-v1 cleanup row maps to an unfinished synthetic greatest sequence and public `unknown`; and every invalid schema or journal-bound payload fails before terminal success. Store-crash proves all exact barriers and a successful post-migration mutation followed by fresh admission, both actual packaged platforms prove their OS primitives and absent-state refusal, and both packaged skill adapters use only the public command and DTO.
...
36. `.lumin` is a no-follow, repository-bound reserved namespace whose state-directory, immutable lock-object, namespace nonce, exact four-kind top-level managed-parent set, and nested cache-quarantine binding are marker/store-bound and revalidated; replacement/swap, aliases, redirected or copied parents, foreign contents, caller writes, external mutation, unauthorized quarantine, and validation-to-eviction substitution cannot enter source evidence or a successful mutation. Public cleanup obeys its exact v2 JSON/exit/stderr/operation contract, authorizes moves before rename, flushes and remanifests full trees plus parent directories before validation/result commit, resumes only through the same operation ID, preserves all bindings, and performs no pathname-based physical deletion. Exact process-death and `pending -> interrupted -> pending` barriers reject every competing active-cache mutation before bytes change and assert the unchanged active-cache, authorization, order, and final durable snapshots.
...
| 9 | `packages_and_skills_share_behavior_contract` | package fixture set plus packaged Codex/Claude adapters | both target package checks plus `lumin-xtask package-check skills` | Windows/Linux query values match; both adapters invoke the same public commands/DTOs with no embedded semantic table, source fallback, private-store read, or migration implementation, generate operation IDs for gate/retention/cache-cleanup mutations, recover each committed result through `operation show`, and route the exact migration-required diagnostic through public `lumin store migrate` before retrying the original command. |
...
| 12 | `all_pages_are_reachable` | `bounded-nested-query` | `lumin-xtask corpus foundation` | Run and gate-revision cursor traversal returns exactly `total` top-level and nested items without following a newer scope. A fixture with 1 library export and 101 test re-exports yields findings 102=100+2, evidence 102=100+2, relations 101=100+1 split at PAGE_SIZE=100. Cursors are bound to exact repository/run or repository/gate/revision scope, fixed page policy, and a canonical nonterminal resume boundary. Cross-run, cross-gate, cross-revision, cross-finding, cross-collection, run-vs-gate, structured payload mutation, nonboundary-anchor, and cross-repository reuse is rejected without page-1 restart and with empty stdout. Immutable run cursors survive a second audit/repository mutation. Versioned `lumin-run-cursor.v2` and `lumin-gate-cursor.v2` payloads use protocol-owned deterministic content binding plus `deny_unknown_fields`; the binding is modification detection, not authentication or authorization. |
| 13 | `default_publication_is_bounded` | output-layout fixture | `lumin-xtask corpus foundation` | Native v13 publishes only the repository state marker, lifecycle lock/store, four immutable top-level managed-parent anchors, the nested cache-quarantine anchor, attempt/run envelopes, canonical evidence store, and latest pointer, with no migration artifact. Migrated v13 contains that bounded layout plus exactly one authorized contiguous journal chain whose unique maximal revision is terminal, including all immutable predecessors, and one retained immutable v12 source; no second, forked, gapped, or unfinished chain, staging name, superseded target object, or extra migration artifact remains. |
...
| 17 | `lifecycle_mutations_are_idempotent` | `lifecycle-operation-idempotency` | `lumin-xtask corpus foundation` and `lumin-xtask corpus foundation --determinism` | Every gate, retention, or cache-cleanup mutation retry returns one committed result. Cleanup `operation show` is v2 with no public-v1 fallback. Exact barriers after delivery allocation and after partial and complete stdout before completion expose `lastDeliveryStatus: "unknown"`. Lower-first completion remains unknown until the greater result commits; greater-first completion remains final after the held lower attempt commits late, with the public v2 projection byte-identical and the private ledger changed only by the lower result. Read-only show preserves state, one pending/interrupted/pending transition occurs per dead cleanup execution lease, and recovery creates no duplicate move or authorization row. |
...
| 31 | `lifecycle_migration_fences_generations` | `lifecycle-store-migration` | `lumin-xtask corpus foundation --store-crash`, `lumin-xtask package-check windows-x64`, `lumin-xtask package-check linux-x64`, and `lumin-xtask package-check skills` | Store-crash public children drive multi-process v12-to-v13 fault injection, preserve one generation, reject/reopen every old-generation late mutation, prove revision zero cannot be adopted without the greatest v12 authorization, and prove initial plus successor publication deaths leave no pending name and exactly the prior contiguous journal or one complete next revision. They exercise both pending-target outcomes—proven absent becomes superseded plus a fresh binding, exact visible becomes published—without requiring a superseded object later; map exact v12 cleanup rows to canonical synthetic sequences or fail before exchange; prove ordinary diagnostic then migration hard-stop for a header-valid/payload-corrupt journal-bound source; reject canonical-source/private-target/root/predecessor substitution; admit only the exact journal-proven canonical-absent intermediate; preserve every object under a real hard-link race; reject a byte-identical no-journal source without deletion; and refuse absent-store initialization without changing any entry. After terminalization they commit an ordinary v13 mutation, then prove fresh admission and no-op migration accept current-v13 integrity without comparing the migration-time target payload while the provenance files remain unchanged. Each actual packaged binary executes root-authorized append-only revision publication and its no-disposition exchange primitive (`renameat2(RENAME_EXCHANGE)` on Linux or handle-relative `SetFileInformationByHandle(FileRenameInfo, ReplaceIfExists = false)` moves on Windows), validates the permanent terminal journal/retired source, recovers binding/exchange/terminal boundaries, and returns the same target-only DTO on retry without another generation advance or cleanup. Both packaged adapters react to the recovery diagnostic by invoking the public migration command, accept only its exact DTO, and retry the original public command without embedded migration logic or private store access. |
...
| 36 | `state_namespace_is_reserved` | `reserved-state-namespace`, `state-namespace-initialization`, `state-lock-replacement-split-brain`, `state-managed-parent-replacement` | `lumin-xtask corpus foundation`, `lumin-xtask corpus foundation --determinism`, `lumin-xtask corpus foundation --store-crash`, plus `lumin-xtask package-check windows-x64`, `lumin-xtask package-check linux-x64`, and `lumin-xtask package-check skills` | Standard and determinism exercise unauthorized self-hashed quarantine, dirty-tree bottom-up durability, both substitution barriers, exact stream failures, empty-cache authenticated quarantine, and public-child deaths after authorization, rename visibility, physical flush, row validation, and result commit. Cleanup show uses v2 with no public-v1 fallback; exact barriers force lower-first and greater-first delivery completion, and a late lower completion leaves the public projection byte-identical while appending only its private evidence. At exact process-death and pending/interrupted/pending barriers a competing cache writer is rejected before mutation; the active-cache bytes, remaining order, authorization states, and final durable snapshot remain unchanged. Same-ID retry proves the exact dead lease through read-only `operation show`; a different ID cannot adopt pending state. Both packaged platforms prove the command/result/exits, bound nested quarantine, parent flushes, and no physical deletion; the skill check proves operation-ID handling. Store-crash also proves the exact private-v1 cleanup-record migration and applicable namespace/bootstrap faults. All lanes preserve one bound state domain and reject foreign state. |
