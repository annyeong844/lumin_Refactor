...
reserved provenance. On current v13 it validates either native no-journal state or the
exact terminal journal/source state and returns without exchanging the store or advancing
its generation.

Revision zero is prepared and flushed on a same-volume handle-owned object whose
unpublished lifetime is automatically disposed. Only that held object may be published
no-replace as `lifecycle-migration.json`; death before publication removes it, while death
after publication but before the parent flush leaves either no root or the complete root.
...
before its private name is visible.
...
survives the hard-stop. Concurrent migration commands serialize; at most one advances the
generation and every follower validates v13 and returns the same response.
...
- `lumin store migrate` is the only private-v1 logical reader, refuses absent state without creating it, uses transaction-scoped handles and generation fencing, publishes a contiguous append-only self-bound journal without a crash-surviving pending name, physically binds the canonical source and private target, and exchanges them through a no-disposition platform primitive; it permanently retains the terminal journal and retired source rather than assuming an impossible Linux hard-link fence, returns the same target-only `ready` response after first success or retry, and prevents an old-generation process from committing after exchange; ordinary repository-state commands route an unfinished journal without decoding its private payload and never migrate on open, while migration alone admits the exact journal-proven canonical-absent intermediate and an unbound, substituted, or no-journal object is foreign;
...
31. `lumin store migrate` is the sole public v12-to-v13 route: ordinary repository-state commands return its exact fail-closed instruction for v12, a matching unfinished journal, or the exact journal-proven canonical-absent intermediate without decoding private-v1 records; a never-initialized repository fails without creating state. Its target-only success DTO is byte-identical after first migration, crash/output recovery, and already-current retry, and v13 retry performs no exchange, journal append, deletion, or generation advance. The command uses transaction-scoped handles and generation fencing, leaves no crash-surviving pending name, publishes each self-bound intent revision at a unique append-only name without replacing its predecessor, binds the canonical source and private target byte/logical and physical identities, and exchanges them through Linux `renameat2(RENAME_EXCHANGE)` or Windows handle-relative `SetFileInformationByHandle(FileRenameInfo, ReplaceIfExists = false)` moves. The terminal journal and retired source remain immutable provenance because supported Linux cannot fence a noncooperating same-UID hard-link creation through disposition. Every crash point has one recovery rule; source, target, or journal substitution and a competing hard-link attempt cannot make Lumin delete a foreign object; an old-generation late writer cannot commit without reopening and revalidation; every committed private-v1 cleanup row maps to an unfinished synthetic greatest sequence and public `unknown`; and every invalid schema or journal-bound payload fails before terminal success. Store-crash proves all exact barriers, both actual packaged platforms prove their OS primitives and absent-state refusal, and both packaged skill adapters use only the public command and DTO.
...
| 13 | `default_publication_is_bounded` | output-layout fixture | `lumin-xtask corpus foundation` | Only the repository state marker, lifecycle lock/store, four immutable top-level managed-parent anchors, the nested cache-quarantine anchor, attempt/run envelopes, canonical evidence store, and latest pointer are published; the migration intent exists only during migration. |
...
| 31 | `lifecycle_migration_fences_generations` | `lifecycle-store-migration` | `lumin-xtask corpus foundation --store-crash`, `lumin-xtask package-check windows-x64`, `lumin-xtask package-check linux-x64`, and `lumin-xtask package-check skills` | Store-crash public children drive multi-process v12-to-v13 fault injection, preserve one generation, reject/reopen every old-generation late mutation, and prove initial plus successor publication deaths leave no pending name and exactly the prior contiguous journal or one complete next revision. They map exact v12 cleanup rows to canonical synthetic sequences or fail before exchange, prove ordinary diagnostic then migration hard-stop for a header-valid/payload-corrupt journal-bound source, reject canonical-source/private-target and predecessor substitution, admit only the exact journal-proven canonical-absent intermediate, preserve every object under a real hard-link race, reject a byte-identical no-journal source without deletion, and refuse absent-store initialization without changing any entry. Each actual packaged binary executes handle-owned append-only revision publication and its no-disposition exchange primitive (`renameat2(RENAME_EXCHANGE)` on Linux or handle-relative `SetFileInformationByHandle(FileRenameInfo, ReplaceIfExists = false)` moves on Windows), validates the permanent terminal journal/retired source, recovers exchange/terminal-publication boundaries, and returns the same target-only DTO on retry without another generation advance or cleanup. Both packaged adapters react to the recovery diagnostic by invoking the public migration command, accept only its exact DTO, and retry the original public command without embedded migration logic or private store access. |
