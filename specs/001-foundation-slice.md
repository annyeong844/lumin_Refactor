...
`lumin cache clean` is the ARCH-002 idempotent disposable mutation. On success it exits `0`, writes exactly `{"schemaVersion":"lumin.cache-cleanup.v1","status":"clean"}` plus the normal newline to stdout, writes nothing to stderr, preserves every canonical record and the bound cache parent/anchor, and leaves that parent with only the anchor. It accepts at most one split-form `--format json`; missing/unknown values, repeated format flags, equals-form flags, positional arguments, or a non-JSON format are malformed exit `2`. Namespace, payload-identity, topology, persistence, or result-delivery failure exits `1`; a pre-transport failure leaves stdout empty, while a stream failure may transfer only a nonauthoritative prefix. The command has no operation ID: a delivery failure may occur after noncanonical payload removal and is recovered by rerunning the same idempotent command. Its public race fixture replaces a validated payload name before deletion and must prove that the substitute and immutable anchor survive an integrity hard-stop.
...
| `reserved-state-namespace` | `.lumin` symlink/junction/reparse/mount parents, nested redirected state parents, foreign/preexisting contents, `RepositoryId` mismatch, caller-declared state writes, physical aliases, and external state mutation each produce the exact malformed/integrity outcome without source evidence or gate success. Public cache cleanup follows the Section 6 command/response/exit/delivery contract, removes nested payloads and remains idempotent, and a barrier-forced validation-to-delete substitution hard-stops without deleting the substitute, cache parent, or anchor. |
