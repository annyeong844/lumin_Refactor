...
| 17 | `lifecycle_mutations_are_idempotent` | `lifecycle-operation-idempotency` | `lumin-xtask corpus foundation` | Every gate, retention, or cache-cleanup mutation retry returns one committed result; read-only `operation show` preserves state, while exact barriers prove one pending/interrupted/pending transition per dead cleanup execution lease and delivery recovery creates no duplicate move or authorization row. |
