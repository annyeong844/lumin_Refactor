...
v13 before success. On current v13 it validates and
returns without replacing the store or advancing its generation. The intent binds the
exact private artifact names, generations, schemas, and SHA-256 of the complete canonical
source/target logical dumps. It remains durable while migration authenticates and removes
those artifacts and flushes their absence; only then is intent removal durably flushed as
the final migration mutation. Concurrent migration commands
...
- `lumin store migrate` is the only private-v1 logical reader, uses transaction-scoped handles and generation fencing, retains its digest-binding intent until private cleanup is durably complete, returns the same target-only `ready` response after first success or retry, and prevents an old-generation process from committing after replacement; ordinary repository-state commands route a live intent without reading its private payload and never migrate on open, while a no-intent private artifact is foreign;
...
| `lifecycle-store-migration` | Public children prove that ordinary repository-state commands reject the exact v12 fixture and every matching live-intent fixture with the `lumin store migrate` instruction and no mutation, then the migration command alone advances or recovers them to ready v13. One process holds the v12 generation token while that command migrates; transaction-scoped handles close before replacement, state-directory/lock bindings remain fixed, the digest-binding intent survives until private-artifact deletion and its parent flush complete, every crash point selects the ARCH-002 recovery rule, all logical records/references survive, and the old process must reopen/revalidate before its late mutation can commit. A live-intent source with valid envelope/header but a corrupted private-v1 logical table first yields the ordinary migration-required diagnostic without any byte change, then makes `lumin store migrate` integrity-hard-stop with the canonical store, intent, source, and generation unchanged. A byte-identical valid v12 source introduced after intent removal is foreign and every command hard-stops without deleting it. The fixture maps pending/interrupted cleanup rows to zero sequences, committed legacy `not-attempted` to unfinished synthetic sequence 1, and committed `succeeded`/`failed` to historical completed sequence 1 plus unfinished greatest sequence 2; every committed mapping initially projects `unknown`, and every invalid legacy shape fails before replacement. Initial success, recovery after replacement/private cleanup/output failure, and an already-current retry emit the same exact `lumin.lifecycle-store-migration.v1` bytes, while v13 retry performs no replacement or generation advance. Both packaged platform binaries execute admission rejection, migration success, intent-held cleanup recovery, and byte-identical no-op retry rather than relying on the development CLI. |
...
31. `lumin store migrate` is the sole public v12-to-v13 route: ordinary repository-state commands return its exact fail-closed instruction for v12 or a matching live intent without reading private-v1 payloads, its target-only success DTO is byte-identical after first migration, crash/output recovery, and already-current retry, and v13 retry performs no replacement or generation advance. The command uses transaction-scoped handles and generation fencing, retains its digest-binding intent until authorized private-artifact deletion is durably flushed, and rejects every no-intent private artifact as foreign. Every crash point has one recovery rule, an old-generation late writer cannot commit without reopening and revalidation, every committed private-v1 cleanup row maps to an unfinished synthetic greatest sequence and public `unknown`, and every invalid schema or intent-bound payload fails before replacement or deletion. Store-crash proves the header-valid/payload-corrupt responsibility split and foreign-copy rejection; both actual packaged platform binaries prove admission, success, intent-held cleanup recovery, and retry.
...
| 31 | `lifecycle_migration_fences_generations` | `lifecycle-store-migration` | `lumin-xtask corpus foundation --store-crash`, `lumin-xtask package-check windows-x64`, and `lumin-xtask package-check linux-x64` | Store-crash public children drive multi-process v12-to-v13 fault injection, preserve one generation, reject/reopen every old-generation late mutation, retain intent provenance through private cleanup, map exact v12 cleanup rows to canonical synthetic sequences or fail before replacement, prove ordinary diagnostic then migration hard-stop for a header-valid/payload-corrupt intent-bound source, and reject a byte-identical no-intent source without deletion. Each actual packaged binary rejects v12/live intent without mutation, successfully runs `lumin store migrate`, recovers the platform replacement and intent-held cleanup boundaries, and returns the same target-only DTO on retry without another generation advance. |
