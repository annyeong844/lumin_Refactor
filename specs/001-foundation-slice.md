...
The first slice freezes the exact checked-in [`resolver-config-semantics.v1` artifact](resolver-config-semantics.v1.json), file SHA-256 `50a4c59b4ff594ad1ef99062d030f1c1fae4159d6aa1fdf697111f4b64c92f48`. That artifact, not this prose projection or an implementation table, owns the configuration container/field-path/value-shape registry. It pins the `typescript@6.0.0-beta` npm tarball and extracted 122-key/shape digest, Node `v24.14.1` package-document commit/hash, supported condition keys, neutral reasons, unsupported limitations, and exports grammar. Every observed semantic field is classified exactly once before any probe:

| Class | First-slice contract |
| --- | --- |
| `SupportedAndModeled` | Only artifact entries with this class are consumed: the supported single `extends` chain; the closed `module`/`moduleResolution`, `baseUrl`, and `paths` rules; and the exact package identity, dependency/workspace, package-type, entry, and `exports-v1` shapes. |
...
`lumin-inventory` parses each exact tsconfig JSONC or strict package JSON payload once into ordered model-owned `ConfigDocument`/`ConfigValue` values with source spans; duplicate keys are malformed. `lumin-resolve` applies the artifact to that tree. No `serde_json::Value`, parser node, or second byte parse crosses the boundary. The exact artifact bytes and generated-table digest participate in `AnalysisContractId`; architecture-check compares the artifact SHA, baseline tarball/document hashes, 122-key digest, disjoint exhaustive classes, shapes, reasons, limitations, conditions, exports grammar, and compiled table. An invocation resolution-profile override changes only `moduleResolution` selection and cannot suppress another unsupported field. `TsconfigSemanticsUnsupported` scopes affected importers to one package when provable, otherwise the workspace. Package `imports` uses `PackageImportsUnsupported`; `typesVersions` or unsupported public/export/package shapes use `PublicSurfaceUnsupported` for the owning package. Every limitation is emitted before target selection and blocks absence in that domain. Observed field paths/shapes/values participate in exact cache keys, `AnalysisInputId`, and semantic-read closure.
...
35. Every Unix/Windows native path round-trips one lossless `repo-path.v1` identity through IDs, ordering, cache, cursors, native NUL input, and machine DTOs; physical aliases may reuse one validated payload/parse without merging logical package/config/role contexts.
...
37. The exact checked-in `resolver-config-semantics.v1` artifact classifies every first-slice resolver field/shape; its pinned source hashes, key digest, reasons, limitations, grammar, and compiled table match, while unsupported affecting or unknown inputs emit scoped incomplete evidence before target probing.
...
| 35 | `repository_paths_are_lossless` | `repo-path-lossless`, `logical-source-physical-aliases` | `lumin-xtask corpus foundation --determinism` plus Windows/Linux package checks | Raw/native names round-trip; symlink/hard-link aliases retain distinct logical contexts, share only compatible payload work, and remain one physical conflict group. |
...
| 37 | `resolver_configuration_fails_closed` | `resolver-config-registry-artifact` plus resolver-registry, module-suffix/custom-condition/root-dirs/typesVersions/export-shape rows | `lumin-xtask corpus foundation` and architecture-check | Exact artifact/source/key/compiled digests match; every field/shape has one class and unsupported inputs cannot create false edges/absence. |
...
| 20 lossless path identity | in scope | Slice AC 35 and native-path plus logical-source/physical-alias corpus/package probes. |
| 21 reserved state namespace | in scope | Slice AC 34/36 and no-follow lock/state replacement fault corpus. |
| 22 resolver configuration honesty | in scope | Slice AC 21/37 and the exact registry-artifact plus unsupported-field corpus. |
