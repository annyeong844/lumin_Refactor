...
| `lifecycle-operation-idempotency` | Pre/post, abandon, pin/unpin, prune-plan creation/confirmation, and cache cleanup use one operation contract: same ID/request joins, safely resumes a protocol-proven interruption, or returns one committed result; conflicting reuse is malformed. `operation show` is read-only; after cleanup child death it shows `pending` until the same-ID mutating retry proves the exact lease dead, exposes one durable `interrupted` barrier/count increment, and installs a fresh `pending` attempt before resuming. Delivery barriers after sequence allocation, after partial or complete stdout but before completion, and after lower-sequence completion with a greater unfinished attempt all project `lastDeliveryStatus: "unknown"`; only the greatest allocated attempt's completion selects `succeeded` or `failed`. Recovery creates no duplicate cache authorization or move and asserts the final operation snapshot. |
...
| 17 | `lifecycle_mutations_are_idempotent` | `lifecycle-operation-idempotency` | `lumin-xtask corpus foundation` and `lumin-xtask corpus foundation --determinism` | Every gate, retention, or cache-cleanup mutation retry returns one committed result. Exact barriers after delivery allocation, after partial and complete stdout but before completion, and after an older completion while a greater attempt is unfinished all expose `lastDeliveryStatus: "unknown"`; the greatest attempt's completion alone selects its final status and operation snapshot. Read-only `operation show` preserves state, one pending/interrupted/pending transition occurs per dead cleanup execution lease, and recovery creates no duplicate move or authorization row. |
