...
Retention is a public, crash-consistent lifecycle operation. It cannot expose a record as deleted before its canonical indexes and payload ownership agree, cannot remove a protected latest/pinned/active-transition reference closure, and has one recoverable outcome at every deletion boundary. A known record in `Pruning` or `Pruned` state remains distinguishable from a never-existing ID through public queries, and independent pins cannot remove one another's protection. Lifecycle-store migration admits mutations in exactly one generation: backend handles are transaction-scoped, replacement is generation-fenced, and every migration crash boundary has one recovery rule. An incompatible but explicitly supported prior lifecycle-store schema is upgraded only through the public `lumin store migrate` command. Ordinary repository-state commands fail closed with that recovery route and never migrate or read a private old-schema payload on open. Intent preparation uses a handle-owned automatically disposed object, so death before no-replace publication cannot leave a named pending or partial intent. The canonical intent self-binds its physical identity and remains durable provenance until every private artifact whose name, role, generation, schema, complete logical digest, physical identity, and one-link state it bound before publication is disposed through the same held object and that absence is flushed. An unbound, substituted, multiply linked, or no-intent private artifact is foreign and no command may adopt or delete it. A successful migration command is safely repeatable after process or output-delivery failure and reports only the fully validated current target schema.
...
19. Concurrent latest publication, recovery, retention confirmation, cache cleanup, and migration serialize through one bound exclusive domain and cannot regress either pointer, lose an independent pointer-field update, strand terminal state behind same-sequence `Running`, publish a pruned target, or duplicate a cache move. Only `lumin store migrate` admits an explicitly supported prior store schema; ordinary repository-state commands never mutate it on open. Intent publication leaves no crash-surviving pending name, every visible private artifact is durably bound by physical identity and one-link state before publication, only entry-to-held-handle final disposition may remove it, unbound/substituted/no-intent artifacts are foreign, and migration/output recovery returns the same validated target-only response without a second replacement.
