...
The public cache-cleanup operation may evict only disposable payload descendants from the active cache. It is idempotent and changes no canonical run, gate, lifecycle, or operation record. Every invocation admits the complete pre-existing cache quarantine before inspecting or mutating active payloads, including an empty-cache retry; each quarantined tree must reproduce the pre-move manifest digest carried by its entry name, and malformed names, digest disagreement, redirects, mount crossings, unsupported entries, multiply linked regular files, or topology disagreement are integrity failures rather than cache misses. Success means the held cache parent contains exactly its original bound anchor, the admitted quarantine is unchanged except for this invocation's durable atomic additions, and the complete state namespace passes final validation; detached payload bytes may remain in that disjoint noncanonical quarantine below the bound trash parent. Before every move, cleanup binds the expected canonical manifest into the no-replace destination name, so a process death between the move and post-move comparison cannot promote a racing substitute into admitted quarantine on retry. Creating the quarantine is durably published in the held trash parent before the first payload move. Cleanup never physically deletes a quarantined object. A payload or nested-child name that changes after validation is never permission to delete the replacement: the winning object is preserved in quarantine and cleanup fails closed instead. Physical reclamation is outside this command and is permitted only through a final disposition primitive bound to the opened object or an enforceable isolation boundary; when neither exists, quarantine is retained. Because cleanup has no canonical mutation result to recover, process death or result-delivery failure is recovered by safely repeating the whole cleanup command rather than by allocating an operation ID.
