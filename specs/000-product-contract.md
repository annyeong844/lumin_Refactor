...
20. Every admitted native repository path/root has one byte-complete canonical identity, strict decoder, and golden machine round trip; distinct Linux byte names and Windows native names cannot merge, and a write to one physical alias expands leases, actual-write attribution, and reanalysis to every admitted logical context without merging them.
...
22. Every inventory/workspace- or resolver-affecting configuration field/shape is present under one owner in the exact reviewed registry artifacts; fields outside their modeled subsets emit the correct semantic-family limitation before ownership or target probing, and artifact/owner-partition/generated-table drift is a contract failure.
