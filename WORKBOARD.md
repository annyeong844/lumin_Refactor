...
- Run and gate retention now use immutable plan/content identities, independent pins, active-transition exclusions, repository-scoped pagination, and idempotent operation records. Confirmation advances `Prepared -> Pruning -> Pruned`, binds canonical tombstones before physical moves, keeps source-or-trash exactly one through recovery, and preserves the first committed operation result while later retries may finish current physical reclamation. Public run/plan/lookup/operation DTOs survive process reopen and expose tombstones instead of plain not-found.

Next implementation order: add named test-build retention crash points and drive every tombstone, canonical-to-trash move, `Pruned` commit, and reclamation boundary through public child processes. Public run/gate lookup, plan show, and operation show must agree after each death; no test may import private store APIs or manufacture lifecycle rows.
