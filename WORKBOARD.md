...
- The store still does not claim the complete ARCH-002 managed-state parent binding, generation fencing, retention, or migration contract. New sources and directory-created descendants are admitted to the write domain and may authorize when their complete current evidence has no adverse lifecycle delta.

Next implementation order: complete the four-kind managed-state parent binding for `attempts`, `runs`, `trash`, and `cache`. Persist each directory/anchor physical identity and parent nonce in the repository marker and store header, revalidate the complete kind-ordered set through held handles around every existing store mutation, and hard-stop replacement, copied state, extra links, redirects, or cross-volume parents before canonical success. Preserve the current operation, gate, publication, and namespace behavior while adding the public child-process replacement corpus required by ARCH-002.
