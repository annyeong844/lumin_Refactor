...
- The store now persists the state-directory identity, immutable one-link lifecycle-lock identity, namespace nonce, and the exact kind-ordered `attempts | runs | trash | cache` directory/anchor/parent-nonce set in canonical lock, marker, anchor, and lifecycle-store headers. Every existing store lock scope carries held state/parent/anchor handles, revalidates the complete binding before and after physical mutations and lifecycle commits, and hard-stops copied/replaced parents, anchor replacement or extra links, redirects, and cross-volume parents before canonical success. Exact nonce-bound pre-marker parent remnants may resume; foreign remnants are never adopted. The store still does not claim the separate canonical `RepositoryId`/root binding, lifecycle-store generation fencing, retention, or migration contracts. New sources and directory-created descendants are admitted to the write domain and may authorize when their complete current evidence has no adverse lifecycle delta.

Next implementation order: complete the canonical `RepositoryId`/root binding and state-directory/lifecycle-lock replacement split-brain corpus before starting lifecycle-store generation fencing. Route the project-owned root identity from `lumin-model`/`lumin-inventory` into `lumin-store` without deriving a second repository identity there, and preserve the completed four-parent binding through every admission and mutation token.
