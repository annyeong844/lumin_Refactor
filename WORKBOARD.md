...
- The lifecycle-store header now persists a nonzero `StoreGeneration`. Store-owned database and read/write transaction wrappers are lifetime-bound to the held namespace guard, so backend handles cannot cross its repository lock scope. Audit attempts and operation sessions retain only opaque generation tokens across analysis; every later physical or logical mutation reopens the canonical store at that generation and returns a typed hard-stop before mutation when replacement advanced it. Replacement tests prove old attempt and gate-operation tokens cannot publish terminal state, while a new session opens the current generation. Copy-on-write migration intent, logical record transfer, replacement recovery, and retention remain unimplemented.

Next implementation order: implement the generation-fenced copy-on-write lifecycle-store migration intent, logical record transfer, durable replacement, and crash recovery protocol using the completed transaction-scoped handles and stale-writer rejection. Do not begin retention publication until migration recovery preserves one authoritative generation at every crash boundary.
