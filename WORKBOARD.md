...
- The store still does not claim the complete ARCH-002 managed-state parent binding, generation fencing, process-death reservation recovery, close-time semantic-read fixed point, total capability delta, abandon, retention, or migration contract. New sources and directory-created descendants are correctly admitted to the write domain but remain typed `Incomplete` when their semantic delta has no implemented owner policy.

Next implementation order: implement capability-owned total lifecycle delta classification for the existing foundation findings and limitations, then add the close-time semantic-input demand fixed point without weakening the new lease, alias-topology, or transition-chain checks. Keep every unsupported branch typed and scope-limited; never recover coverage by a simplified fallback or policy mute.
