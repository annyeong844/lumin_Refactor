...
- Close-time resolver analysis now retains one owned extraction session across `NeedsInputs` steps. Each new config path is transactionally recorded as an operation-scoped semantic-read reservation before inventory captures it; active/provisional writers make that close attempt typed `Incomplete`, and a later writer cannot cross a live reservation. Successful closure persists the exact finished protected read set on the revision and gate, while newly demanded read-only paths remain distinct from actual writes and exact transition snapshots preserve read-set growth.
- The store still does not claim the complete ARCH-002 managed-state parent binding, generation fencing, process-death reservation recovery, pre-write demand-by-demand semantic-read reservation, abandon, retention, or migration contract. New sources and directory-created descendants are admitted to the write domain and may authorize when their complete current evidence has no adverse lifecycle delta.

Next implementation order: extend the same reservation-before-capture owner-step protocol to pre-write provisional admission without weakening the lease, alias-topology, transition-chain, close-time fixed point, or total-delta checks. Keep every unsupported branch typed and scope-limited; never recover coverage by a simplified fallback or policy mute.
