...
- The store still does not claim the complete ARCH-002 managed-state parent binding, generation fencing, process-death reservation recovery, close-time semantic-read fixed point, abandon, retention, or migration contract. New sources and directory-created descendants are admitted to the write domain and may authorize when their complete current evidence has no adverse lifecycle delta.

Next implementation order: add the close-time semantic-input demand fixed point without weakening the lease, alias-topology, transition-chain, or total-delta checks. Keep every unsupported branch typed and scope-limited; never recover coverage by a simplified fallback or policy mute.
