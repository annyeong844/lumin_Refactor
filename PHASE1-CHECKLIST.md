...
| --- | ---: | ---: | ---: | --- |
| Standard | 86 | 46 | 40 | Last full aggregate: 40 passed, 0 failed, 46 unmapped. The six additional mapped rows—`logical-source-physical-aliases`, `repo-path-codec-golden-vectors`, `tsconfig-aliases`, `tsconfig-extends-specifier-selection`, `workspace-package-extends-tsconfig-field`, and `tsconfig-module-suffixes-unsupported`—passed with their public-binary markers; P1-70 owns the next full aggregate. |
| Determinism | 86 | 46 | 40 | Last full aggregate: 40 passed, 0 failed, 46 unmapped. The same six additional rows passed with nonempty canonical semantic captures across repeated default jobs and `jobs=1`; P1-70 owns the next full aggregate. |
| Store crash | 10 | 4 | 6 | Mapping count only; Phase 1 exit still requires the complete lane. |
| **Total execution obligations** | **182** | **96** | **86** | This total deliberately counts each required lane execution. |
...
- [x] Keep determinism invocations paired with all 46 currently mapped standard
  rows.
