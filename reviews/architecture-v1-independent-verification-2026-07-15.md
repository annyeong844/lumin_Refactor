...
| Latest externally reverified candidate | `74dded225d098253fa46b2972c6d18547e739581`; all 14 supplied packet bytes matched, H-10 passed at document level, H-09/H-11 reopened under R3, and exact Git binding remained pending |
| Current resolution candidate | `491f47824023253ea61001b4678d2e391db49268`; independent verification still required |
...
| Latest externally reverified input manifest/report | `de1e4682a5cecbeb0ed7474db9fc25d9240baf4b739eba7abc442882d7879a4c` / `c5d4bf25a8d2b04686281ff3b1eef28b5d3eac8fa4bdd88e3c5b30d913110e0c` |
| Verification result | `74dded2` remained blocked: H-01 through H-08, H-10, and H-12 passed at document level or retained their measured-proof gate; R3 reopened H-09 for the incomplete path codec and alias-write meaning, H-11/B-03 for resolver conditions, workspace ownership, mismatch families, and exports-pattern precedence; B-07 and measured gates remained pending |

The first report verified the exact first amended revision. Later reports verified supplied byte manifests but could not prove loose uploads byte-identical to the declared Git commits. The `c4af6ff` and `74dded2` packets included both canonical Korean files and every declared packet hash matched, but neither proved exact Git-tree binding. This record does not call the current resolution independently verified until a separate reviewer checks the exact current candidate checkout.
...
| H-09 path encoding and logical/physical deduplication | External REOPEN, then R3 REOPEN -> accept finding | The exact `repo-path-semantics.v1` artifact owns every relative/root tag, framing byte, native conversion, canonical rejection, Base64 rule, and golden vector. `LogicalSourceId`, `PhysicalFileIdentity`, and `PayloadSnapshotId` remain separate, while `PhysicalAliasWriteClosure` expands reservations, leases, actual-write attribution, and per-context reanalysis to every admitted alias. |
...
| H-11 exhaustive resolver-config handling | External REOPEN, then R3 REOPEN -> accept finding | Disjoint `inventory-config-semantics.v1` and `resolver-config-semantics.v1` artifacts pin TypeScript, Node, and pnpm bytes; owner-specific field/shape families; profile feature/condition sets; pnpm precedence; exact exports comparator/segment rules; and generated-table checks without an `inventory -> resolve` edge. |
...
| R3-H11-01 profile-specific package conditions | External REOPEN -> accept finding; independent PASS pending | The resolver artifact disables `exports`/`imports` for legacy `node`, excludes `node`/`require` from bundler, gives Node16/NodeNext explicit import/require value and type sets, and records Lumin's separate value/type evidence lanes as a versioned deliberate divergence. |
| R3-H11-02 pnpm workspace registry and Cargo owner | External REOPEN -> accept finding; independent PASS pending | The inventory artifact owns package workspace and pnpm `packages` shapes, malformed/unsupported outcomes, same-directory precedence, and `workspace-glob.v1`; inventory emits model facts and resolver consumes those facts plus the one inventory-parsed `ConfigDocument`, with no reverse Cargo edge or duplicated policy. |
| R3-H11-03 package-field mismatch family | External REOPEN -> accept finding; independent PASS pending | Resolver and inventory artifacts map each owned field's bad shape to its semantic family: importer format, public surface, package identity/privacy, dependency ownership, workspace ownership, or unsupported pnpm dependency semantics. |
| R3-H11-04 exports pattern precedence | External REOPEN -> accept finding; independent PASS pending | The resolver artifact pins Node's `patternKeyCompare`, exact-first selection, UTF-16 specificity keys, source-order tie break, invalid/deprecated segment sources, the explicit stricter Lumin segment policy, and overlapping-pattern vectors. |
| R3-H09-01 byte-complete path codec | External REOPEN -> accept finding; independent PASS pending | `repo-path-semantics.v1` fixes `LUMRPATH`/`LUMRROOT`, big-endian widths, every component/root/physical tag, Windows WTF-16/WTF-8, RFC 4648 padded Base64, canonical decoder rejection, and 14 exact path/root/I/O vectors. |
| R3-H09-02 physical-alias write closure | External REOPEN -> accept finding; independent PASS pending | ARCH-002 reserves the physical key before alias enumeration, reservation-expands every admitted logical alias before capture, exposes expansions to the caller, reanalyzes each logical context, and gives alias topology changes one deny/incomplete/stale rule. |

`Accept finding` records the architecture-authoring decision. It is not an independent `PASS`.

## Author-Side Preflight

The architecture-authoring session re-read the current candidate before packaging it for external review. This is not independent verification. For `491f478`, author-side adversarial review preserved the externally coherent H-01 through H-08/H-10/H-12 repairs and checked every R3 H-09/H-11 amendment across exact path bytes, physical-alias writes, profile conditions, pnpm ownership, mismatch families, and exports-pattern selection.
...
| Canonical path codec | The exact checked-in `repo-path-semantics.v1` artifact fixes every relative/root byte, tag, width, endian rule, native NUL-stream conversion, DTO Base64, rejection rule, and golden vector; only one model-owned generated codec implements it. |
...
| Resolver compatibility closure | The resolver artifact pins TypeScript npm/source and Node doc/resolver bytes, all 122 TypeScript option key/shapes, profile-specific package features and conditions, semantic-family mismatch limitations, exact `patternKeyCompare`/segment rules, and generated-table digest. |
| pnpm workspace closure | The inventory artifact pins pnpm documentation bytes and owns `pnpm-workspace.yaml` field/shape handling, valid same-directory precedence, missing/malformed behavior, and `workspace-glob.v1`; unsupported dependency semantics fail closed under inventory-owned limitations. |
...
This author-side preflight found no remaining known H-01 through H-12 or R3 contradiction after those repairs. Independent local encoders reproduced all 10 path/root and four native-I/O vectors; resolver checks reproduced six profiles, four condition selections, and three pattern selections. Exact downloads matched the pinned TypeScript tarball/`typescript.js`/resolver source, Node package/resolver source, and pnpm workspace-document hashes. That is not a freeze approval: the exact candidate still requires independent checkout review and all measured gates below. Earlier D-02, D-03, and D-04 remain externally passed; D-01 remains covered by the externally passed profile contract.
...
| Repository-path codec artifact | `specs/repo-path-semantics.v1.json` is the byte-complete machine owner and freeze-manifest entry; architecture-check compares exact bytes, the sole generated codec, canonical rejection, and every path/root/native-I/O vector. |
| Inventory/resolver registry artifacts | `specs/inventory-config-semantics.v1.json` and `specs/resolver-config-semantics.v1.json` are disjoint machine owners and freeze-manifest entries; architecture-check compares exact upstream bytes, field partitions, shape families, profile conditions, pnpm precedence, comparator/segment rules, generated tables, and Cargo edges. |
...
The independent reviewer must inspect Git commit `491f47824023253ea61001b4678d2e391db49268`, not this later ledger commit and not a loose export. The 16-file candidate manifest is sorted by repository-relative path using ordinal ordering; each line is `sha256`, two spaces, path, LF. Its SHA-256 is:

```text
2988ff4d2f5fd054f60a5453d836d5974d3ea141e8d833416ce13929b4216cbc
```
...
5a2623f59b1b307615fd77d58f1e837fb41a90bef0b338279dbc2615d36191d8  architecture/000-system-blueprint.md
5a3fdf57e5d85f4eca2bd11fcd17bdc0cb0b3b7518be4c861d25aaea8345618b  architecture/001-execution-and-ownership.md
ee8885c3e5b69636a5bc29539bd9922ca0010d8bf0005358bceea513f6d0835e  architecture/002-evidence-and-write-gate.md
ee4628e495b2f439cbf11dd1282560d615caf42c325cac3752b377b9ecb03960  specs/000-product-contract.md
95c52218940093a75b2ee697ebed57e5b9e2434aa1801eaa2ca6a717b936aa16  specs/001-foundation-slice.md
d5278e0076fd1d6d10bc300a7da758b4284af369c9a9fc60eeede17a333fca94  specs/inventory-config-semantics.v1.json
05aa639d85ce3cb553e1a7a716b892b3e2112ba626488e15ba204e390a36684c  specs/repo-path-semantics.v1.json
13651a6e22b564b6529a941786a850dd4e864825ecf909216811759fe9503a43  specs/resolver-config-semantics.v1.json
...
The reviewer must report `PASS`, `REOPEN`, or a new finding for H-01 through H-12, R3-H09-01, R3-H09-02, R3-H11-01, R3-H11-02, R3-H11-03, R3-H11-04, their reopened B/G/E/C predecessors, the packaged-skill and process-reopen proofs, and B-07 exact revision binding. It must also check that:
...
- every bad package/pnpm field shape emits its inventory/resolver semantic-family limitation, pnpm workspace precedence and malformed behavior are unique, and unsupported affecting input fails closed before ownership or target probing;
- exact-first exports selection, overlapping one-star keys, Node `patternKeyCompare`, invalid segment handling, and the explicit stricter Lumin segment rule match the resolver artifact and corpus vectors;
...
1. Independently re-verify exact commit `491f47824023253ea61001b4678d2e391db49268` using the packet above, including both canonical Korean sources and all three exact machine artifacts from that Git tree.
