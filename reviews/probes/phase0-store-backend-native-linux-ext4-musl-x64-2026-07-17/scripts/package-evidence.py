#!/usr/bin/env python3
"""Validate and package native Linux ext4 GNU/musl Phase 0 store evidence."""
...
def main() -> None:
    ...
    for mode, spec in MODES.items():
        ...
        host_uname = str(build.get("host_uname", ""))
        filesystem = str(build.get("filesystem", ""))
        if "ext4" not in filesystem.split():
            raise RuntimeError(f"{mode} build is not bound to ext4: {filesystem}")
        if "microsoft" in host_uname.lower() or "wsl" in host_uname.lower():
            raise RuntimeError(f"{mode} build ran under WSL instead of native Linux")
    ...
    summary = {
        "probe_id": "lumin-store-native-linux-ext4-musl-x64-evidence-summary-v1",
        "scope": "native-linux-ext4-gnu-musl-phase0-evidence",
        "status": "PASS",
        ...
        "native_host": {
            "uname": build_reports["gnu"]["host_uname"],
            "filesystem": build_reports["gnu"]["filesystem"],
        },
        "measurements": measurements,
        "interpretation": [
            "Correctness passed for the exact GNU and musl case sets on this native Linux ext4 host.",
            ...
            "This packet completes the native Linux store candidate comparison but does not itself select the backend.",
        ],
        "pending": [
    ...
    summary_name = "native-linux-ext4-musl-x64-summary.json"
