...
`lumin-resolve` owns the closed [`resolver-config-semantics.v1` artifact](../specs/resolver-config-semantics.v1.json), file SHA-256 `14141d19c8f7dde7bf15cac711ba4f51e7fa10e7f11a27fb40bd77051337235b`. It pins `typescript@6.0.0-beta` package/source bytes, the extracted 122-key compiler-option set and shape digest, Node `v24.14.1` package/resolver bytes, profile-specific package-feature applicability and condition sets, workspace-package `tsconfig` selection, `typings`/`types` precedence, field-family mismatch limitations, and the exact exports pattern/target-lowering rules. Unknown `compilerOptions`/tsconfig keys, unknown package condition keys, unknown shapes beneath an applicable resolver-owned field, and future affecting fields absent from this exact artifact are unsupported rather than presumed neutral. A feature disabled for the selected profile is `NotConsultedForProfile`: its raw manifest identity remains an analysis input, but resolver neither validates its shape nor emits an affecting limitation. Inventory-owned package fields are explicitly delegated to its disjoint artifact, not treated as resolver-neutral or reimplemented. The artifact byte identity and generated resolver-table digest participate in `AnalysisContractId`; every observed field/value identity participates in `AnalysisInputId` and semantic-read closure. Architecture-check rejects any generated table, baseline digest, reason, shape, applicability state, condition set, target-lowering vector, comparator, or owner partition that differs from the checked-in artifacts.
...
18. Every first-slice inventory/workspace- or resolver-affecting configuration field/shape is owned by exactly one checked-in artifact or emits its semantic-family limitation; both generated tables, owner partition, baselines, conditions/comparator, and artifact digests must match byte-for-byte without an `inventory -> resolve` edge.
