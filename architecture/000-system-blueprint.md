...
| `StoreGeneration`, `MigrationIntent`, and `MigrationArtifactBinding` | `lumin-store` project API/private values | `lumin-store` allocates the next generation under the exclusive repository migration lock; every transaction is fenced to the generation of the backend handle it opened. The intent self-binds its canonical physical identity and binds each private artifact's exact name, role, generation, schema, logical digest, physical identity, and one-link state before that artifact becomes visible; only the store's entry-to-held-handle disposition protocol may remove a bound object. |
