...
`RepoPath` is a component sequence, never a Unicode `String`. The exact checked-in [`repo-path-semantics.v1` artifact](../specs/repo-path-semantics.v1.json), file SHA-256 `05aa639d85ce3cb553e1a7a716b892b3e2112ba626488e15ba204e390a36684c`, owns every byte of the relative/root codecs, canonical decoder, native NUL-stream conversion, DTO Base64, and golden vectors. Its canonical bytes are the sole lexical key for logical source IDs, ordering, hashes, leases, cache keys, and cursor anchors; observed physical identity and parent-specific comparison behavior remain separate facts. The absolute repository root uses the artifact's separate platform prefix/volume and physical identity in `RepositoryRootIdentity`; it is never squeezed into a relative `RepoPath` or display string. Protocol display text cannot be converted back into identity.
...
- the closed [`inventory-config-semantics.v1` artifact](../specs/inventory-config-semantics.v1.json), file SHA-256 `d5278e0076fd1d6d10bc300a7da758b4284af369c9a9fc60eeede17a333fca94`, and its generated inventory table for package identity/privacy, dependency ownership, package workspaces, and pnpm workspace membership;
...
`lumin-resolve` owns the closed [`resolver-config-semantics.v1` artifact](../specs/resolver-config-semantics.v1.json), file SHA-256 `13651a6e22b564b6529a941786a850dd4e864825ecf909216811759fe9503a43`. It pins `typescript@6.0.0-beta` package/source bytes, the extracted 122-key compiler-option set and shape digest, Node `v24.14.1` package/resolver bytes, profile-specific package-feature and condition sets, field-family mismatch limitations, and the exact exports pattern/segment rules. Unknown `compilerOptions`/tsconfig keys, unknown package condition keys, unknown shapes beneath a resolver-owned field, and future fields absent from this exact artifact are unsupported rather than presumed neutral. Inventory-owned package fields are explicitly delegated to its disjoint artifact, not treated as resolver-neutral or reimplemented. The artifact byte identity and generated resolver-table digest participate in `AnalysisContractId`; every observed field/value identity participates in `AnalysisInputId` and semantic-read closure. Architecture-check rejects any generated table, baseline digest, reason, shape, condition set, comparator, or owner partition that differs from the checked-in artifacts.
