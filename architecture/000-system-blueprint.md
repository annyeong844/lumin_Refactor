...
- logical source, physical file, payload snapshot, physical-alias write closure, symbol, use, span, and package identifiers;
...
- the closed `FindingDisposition = ReviewCandidate | ReviewOnly { reason }` value used only for remediation eligibility;
- completeness and opacity states.

`RepoPath` is a component sequence, never a Unicode `String`. The exact checked-in [`repo-path-semantics.v1` artifact](../specs/repo-path-semantics.v1.json), file SHA-256 `ee686f81164ff40b281483afaae591793964cc576afaca0ce7b5b51a6798b4a6`, owns every byte of the relative/root codecs, canonical decoder, native NUL-stream conversion, `RepoPathDto`/`RepositoryRootDto` Base64, and golden vectors. Its canonical bytes are the sole lexical key for logical source IDs, ordering, hashes, leases, cache keys, and cursor anchors; observed physical identity and parent-specific comparison behavior remain separate facts. The absolute repository root uses the artifact's separate platform prefix/volume and physical identity in `RepositoryRootIdentity`; it is never squeezed into a relative `RepoPath`, structured ad hoc DTO, or display string. Protocol display/readable text cannot be converted back into identity.

`LogicalSourceId`, `PhysicalFileIdentity`, and `PayloadSnapshotId` are distinct. Inventory creates one logical source for each admitted lexical `RepoPath` and source kind, so package ownership, controlling configuration, scan role, resolver profile, findings, and gate reads remain path-contextual. Several logical sources may name one physical file. Physical identity establishes containment, alias conflicts, cycle prevention, and safe read reuse only; it never selects a representative logical source. A payload snapshot identifies exact captured bytes plus the physical observation used to validate them. Owners may reuse one payload/parse result for compatible parse modes, but they lower a separate logical-source fact envelope and resolve every source use in that logical source's context. For a write gate, inventory derives one `PhysicalAliasWriteClosure` containing every admitted logical alias and its physical conflict key; all members are leased and reanalyzed while the caller-declared set remains separately visible.
...
- run, capability, finding, diagnostic, metric, and limitation records, preserving every grounded finding regardless of its remediation disposition;
...
- the closed [`inventory-config-semantics.v1` artifact](../specs/inventory-config-semantics.v1.json), file SHA-256 `ebca37c3b33f8e4d92ea29e0bcdc51b7cd5ea04a453c4c469a89072f3d2fac02`, and its generated inventory table for exact package identity/duplicate rejection, privacy, dependency ownership, package workspaces, pnpm workspace membership, and valid-but-unsupported pnpm value recognition;
...
`lumin-inventory` emits model-owned package identity/privacy, dependency ownership, workspace membership, and nearest-manifest facts. `lumin-resolve` consumes those facts through engine orchestration and lowers only resolver-owned package entry/exports semantics into `PackageSurfaceDeclaration`; neither crate depends on the other or duplicates the other's registry. The graph consumes the resulting declarations but never interprets `package.json` fields itself.
...
`lumin-resolve` owns the closed [`resolver-config-semantics.v1` artifact](../specs/resolver-config-semantics.v1.json), file SHA-256 `41ffa3dcc108e74dca351b4f3a5fa182090e1481ed6d8333235f38f0459a29a1`. It pins `typescript@6.0.0-beta` package/module-resolver/config-parser bytes, the extracted 122-key compiler-option set and shape digest, Node `v24.14.1` package/resolver bytes, exact `extends-specifier-selection.v1`, profile-specific package-feature applicability and condition sets, workspace-package `tsconfig` selection, `typings`/`types` precedence, field-family mismatch limitations, and the exact exports pattern/target-lowering rules. Unknown `compilerOptions`/tsconfig keys, unknown package condition keys, unknown shapes beneath an applicable resolver-owned field, and future affecting fields absent from this exact artifact are unsupported rather than presumed neutral. A feature disabled for the selected profile is `NotConsultedForProfile`: its raw manifest identity remains an analysis input, but resolver neither validates its shape nor emits an affecting limitation. Inventory-owned package fields are explicitly delegated to its disjoint artifact, not treated as resolver-neutral or reimplemented. The artifact byte identity and generated resolver-table digest participate in `AnalysisContractId`; every observed field/value identity and staged relative-extends hit/miss participates in `AnalysisInputId` and semantic-read closure. Architecture-check rejects any generated table, baseline digest, reason, shape, extends dispatch/candidate vector, applicability state, condition set, target-lowering vector, comparator, or owner partition that differs from the checked-in artifacts.

`lumin-inventory` reads each configuration payload once and lowers strict package JSON, tsconfig JSONC, or the supported pnpm-workspace YAML subset into model-owned `ConfigDocument`/`ConfigValue` values with source order and spans. Duplicate keys are malformed. Inventory applies only its ownership artifact and emits model facts; `lumin-resolve` consumes those facts plus the project-owned config tree and applies only its resolver artifact. Parser nodes, `serde_json::Value`, YAML parser types, and a second unparsed read never cross the crate boundary.
...
- `lumin-dead`: export-level and definition-level liveness classification plus the owner-authored `FindingDisposition`; it cannot turn a grounded finding into a hidden or absent row.
...
Owns immutable run persistence, exact-input cache persistence, write-gate and cache-cleanup operation records, no-follow admission of the reserved `.lumin` namespace, its immutable state-directory/lock/top-level-parent/nested-quarantine binding set, and the exclusive catalog-publication guard shared by latest publication/recovery, retention confirmation, cache cleanup, and migration. ARCH-002 selects exact `redb 4.1.0` as the sole production persistence engine; its API, physical cache schema, migrations, managed-parent and nested-quarantine anchor/directory-handle operations, and locking primitives are private. Capability owners define semantic cache keys; only `lumin-store` calls the backend API or commits storage.
...
| `AnalysisContractId` | `lumin-model` | `lumin-engine` derives it only from the ordered software semantic versions and exact artifact identities of the profile, inventory/workspace registry, path codec, scan contracts, extractors, resolver registry, graph, and selected rules. |
...
| `RepoPath`, `RepositoryRootIdentity`, their canonical bytes, and path comparison facts | `lumin-model` | `lumin-inventory` lowers native OS paths/roots through the exact `repo-path-semantics.v1` codec and observes physical identity/comparison behavior; no other crate reconstructs identity from display text. |
| `LogicalSourceId`, `PhysicalFileIdentity`, `PayloadSnapshotId`, and `PhysicalAliasWriteClosure` | `lumin-model` | `lumin-inventory` allocates one logical identity per admitted lexical source context, records physical aliases separately, derives a payload identity only from exact captured bytes plus their validated physical observation, and expands a declared existing write to every admitted alias plus its physical conflict key. |
| `RepositoryId` | `lumin-model` | `lumin-inventory` derives it from the canonical root and repository identity inputs. |
| `ConfigDocument`, `ConfigValue`, inventory package/workspace facts, and package-surface facts | `lumin-model` | `lumin-inventory` parses each exact config snapshot once, applies only `inventory-config-semantics.v1`, and emits ownership facts; `lumin-resolve` applies only `resolver-config-semantics.v1` to the same project values plus model-owned inventory facts. |
| `AttemptId`, `RunId`, `GateId` | `lumin-model` | `lumin-store` allocates and persists them. |
| `OperationId` | `lumin-model` | The caller creates it before a mutating gate, retention, or cache-cleanup command; `lumin-store` binds it to one repository-scoped request digest, recoverable in-progress state, and committed result. |
| `CacheEvictionAuthorizationSetId` | `lumin-model` | `lumin-store` derives it from the canonically ordered repository-scoped cache-eviction authorization rows; it identifies one exact quarantine authorization set without copying its rows. |
| `CacheCleanupOperationRecord` and `CacheEvictionAuthorization` | `lumin-evidence` | `lumin-store` creates and validates them transactionally before/after the ARCH-002 physical move; no CLI, engine, or adapter reconstructs authorization from filenames or manifests. |
...
| `FindingDisposition` and its stable reason | `lumin-model` | The owning capability assigns `ReviewCandidate` or `ReviewOnly` only after producing a grounded finding. `lumin-evidence` persists it as finding metadata; store, protocol, projections, and skills cannot reinterpret it as `Muted`, `Suppressed`, or absent. |
...
| `RepoPathDto`, `RepositoryRootDto`, and escaped path/root display | `lumin-protocol` | The protocol always carries canonical `repo-path.v1` or `repository-root.v1` bytes and may add artifact-defined readable/display projections; decoding validates canonical bytes and rejects disagreement instead of trusting a projection. |
...
| `StoreGeneration`, `MigrationRootAuthorization`, `MigrationIntentRevision`, `MigrationArtifactBinding`, `MigrationBindingEvent`, and `MigrationExchangeProtocol` | `lumin-store` project API/private values | `lumin-store` allocates the next generation under the exclusive repository migration lock; every transaction is fenced to the generation of the backend handle it opened. Canonical v12 commits the append-only authority for the exact root object before publication. The migration journal is a contiguous append-only chain of immutable revisions; its closed binding-event fold distinguishes observed source, pending/published/superseded target, exchange, retained immutable source, and canonical mutable target while preserving every predecessor. Artifact bindings name the exact source/target roles, pre/post-exchange names, generations, schemas, migration-time byte/logical digests, physical identities, and publication-time one-link states. Exchange uses Linux `renameat2(RENAME_EXCHANGE)` or Windows handle-relative `SetFileInformationByHandle(FileRenameInfo, ReplaceIfExists = false)` moves and never disposes a published object. The terminal chain and retired v12 source remain immutable provenance; normal target mutations are validated by immutable header/provenance facts plus current-v13 integrity rather than the historical target digest, and no project type claims to exclude a noncooperating same-UID `linkat`. |
...
14. Cache replay, observation binding, finding disposition, gate delta classification, retention plans, pins, and collection ordering use the authority table without duplicate policy in engine, store, protocol, CLI, or skill adapters.
...
17. Every native repository path/root reproduces the exact path-codec artifact and wire vectors; logical source identity survives physical aliasing, while a declared alias write expands its lease/reanalysis closure and payload snapshots may deduplicate only byte/parse work.
18. Every first-slice inventory/workspace- or resolver-affecting configuration field/shape is owned by exactly one checked-in artifact or emits its semantic-family limitation; both generated tables, owner partition, baselines, exact `extends` selector/candidate order, conditions/comparator, and artifact digests must match byte-for-byte without an `inventory -> resolve` edge.
19. Only `lumin-store` opens the reserved `.lumin` namespace or acquires `CatalogPublicationGuard`; bound state-directory/lock identities, the exact four-kind top-level managed-parent set, and the nested cache-quarantine binding prevent replacement split brain or copied-parent adoption, and publication/retention/migration/cache code cannot bypass revalidation or the exclusive guard.
