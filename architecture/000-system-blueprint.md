...
| `StoreGeneration`, `MigrationIntent`, `MigrationArtifactBinding`, and `MigrationNamespaceIsolation` | `lumin-store` project API/private values | `lumin-store` allocates the next generation under the exclusive repository migration lock; every transaction is fenced to the generation of the backend handle it opened. The intent self-binds every canonical revision and binds the exact roles, pre/post-exchange names, generations, schemas, logical digests, physical identities, and one-link states of the canonical source and private target. Only the store's operating-system isolation authority may span final validation, identity-preserving exchange or held-object disposition, absence/reopen proof, and parent flush; it fences both entry substitution and new hard links through that interval. |
