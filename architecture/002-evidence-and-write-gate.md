...
.lumin/
  ...
  trash/
    ...
    cache-evictions/         # reserved noncanonical cache-quarantine family
      <invocation-id>.<ordinal>.<manifest-sha256>  # detached manifest-bound tree
  cache/
    namespace.anchor         # immutable managed-parent bootstrap
...
`.lumin` is a product-owned reserved namespace, including every physical alias and descendant. It is never authored source, a scan target, or a legal gate write. The namespace has three global immutable bootstrap values: `StateDirectoryIdentity`, `LifecycleLockIdentity`, and a cryptographically random `StateNamespaceNonce`. It also has the closed managed-parent kinds `Attempts`, `Runs`, `Trash`, and `Cache`. Each kind owns one immutable `ManagedStateParentBinding { kind, directory_physical_identity, anchor_physical_identity, parent_nonce }`; `namespace.anchor` is a create-new/no-follow regular one-link object inside that exact parent. Physical identities include the platform filesystem/volume and stable object identity, while global and parent nonces distinguish reincarnation even if an object identifier is reused. The four parents and anchors are never renamed, replaced, migrated, linked, trashed, or retention-eligible. Cache payload descendants are disposable; the cache parent and anchor are not. Repository open performs a no-follow namespace admission before reading or creating canonical state:

1. Open the canonical repository root by stable directory handle and open or create its direct `.lumin` child without following a symlink, junction, mount-style reparse point, or later alias. Capture the state-directory physical identity. The state directory and all four managed parents must be real directories on the same state filesystem/volume required by their atomic-rename protocols.
...
3. For admission inspection, open each exact managed-parent entry relative to the held state-directory handle, then open its `namespace.anchor` relative to the held parent handle. Require a real same-volume directory and a regular one-link anchor. Each immutable anchor header binds the global bootstrap values plus its parent kind, held directory physical identity, held anchor physical identity, and random parent nonce. `repository.json` and the current `lifecycle.store` header bind the same global values and the exact four-kind `ManagedStateParentBinding` set in kind order; missing, duplicate, or extra kinds are invalid. Close these admission-only parent/anchor handles before lock acquisition; they authorize no read or mutation.
4. Immediately before acquiring either side of `lifecycle.lock`, re-open the root `.lumin` and state-directory lock entries without following and prove that they still name the held global objects. After acquisition, open/re-open every managed-parent and parent-anchor entry, prove that all entries name the held objects, and validate the marker, anchors, and current store header through those handles. Repeat the complete global/parent proof before and after each physical mutation or canonical commit and before lock release. Only the post-acquisition complete proof may create a shared transaction token or exclusive `CatalogPublicationGuard`.
5. After canonical marker/store publication, every attempt publication, run publication/recovery, retention move/reclamation, cache mutation, lifecycle commit, pointer replacement, and migration replacement uses only store-owned APIs carrying the complete validated binding token. Immediately before and after its physical mutation or canonical commit, it repeats the global and four-parent entry-to-held-handle proof. A symlink, junction, reparse point, mount crossing, extra anchor/lock link, parent or anchor replacement, marker/anchor change, missing/extra kind, or identity/nonce disagreement is a store-integrity hard-stop.
...
Initial namespace publication has one recovery order: create/open the real `.lumin` directory and capture its identity; create-new the regular one-link `lifecycle.lock`; allocate the namespace nonce, write/flush its global bootstrap header, and capture its identity; acquire its exclusive side and revalidate both global directory entries; create each real managed parent and its regular one-link anchor, allocate the parent nonce, write/flush the anchor header, and capture the directory/anchor identities; flush every parent and `.lumin`; publish `repository.json` with the global binding and exact four-parent set by flushed temporary file plus atomic rename and parent flush; then create `lifecycle.store` with the same complete binding. The pre-marker exclusive bootstrap token authorizes only this initialization/recovery sequence and is not a `CatalogPublicationGuard`; no ordinary state success can publish until the full post-acquisition proof can yield that guard. A crash before the marker leaves no canonical repository state; recovery may remove only exact global- and parent-nonce-bound initialization objects whose headers/identities agree with the interrupted bootstrap and then restart. A crash after the marker leaves that matching complete binding authoritative and resumes the matching store idempotently. Any other pre-marker entry, replacement lock/parent/anchor, copied directory, or post-marker identity/schema/nonce disagreement is foreign state, not an initialization remnant.

Simple lock-file, state-directory, managed-parent, or parent-anchor replacement cannot form a second accepted state domain: a new object cannot satisfy the old marker/store physical identity and parent nonce set, and copied headers retain the old directory/anchor binding. A process already holding a displaced object must fail the next complete binding check, roll back any open backend transaction, publish no pointer or success, and return an integrity hard-stop. A physical write that raced between the pre- and post-mutation checks is not a canonical committed result: the repository remains integrity-stopped and no Lumin process may accept either parent domain until recovery proves one matching marker/lock/store/anchor binding. Architecture v1 does not claim protection against an actor that can forge and replace every bound canonical record; such a rewrite is deliberate store tampering, not a second valid Lumin state domain.
...
The repository lock order is fixed: root handle -> marker-bound state-directory handle -> marker-bound `lifecycle.lock` -> held four-kind managed-parent/anchor set -> transaction-scoped backend handle/transaction. The pre-acquire global proof never retains a managed-parent handle across lock acquisition; initialization creates parents only after it owns the exclusive lock. Ordinary transactions acquire the shared side and then create a complete validated binding token before opening the backend. Publication, latest recovery, latest-sensitive retention confirmation, and migration enter through the already-held exclusive `CatalogPublicationGuard` token and must not reacquire the shared side. No code acquires `lifecycle.lock` while holding a parent/backend handle or keeps a backend handle after releasing the lock. Publication may close its backend transaction and perform the pointer file replace while retaining the exclusive guard; retention closes its confirmation transaction before later trash moves. Both retain the relevant held parent handles and repeat the complete Section 2.0 proof before and after their physical mutation. There is no scan lock in this order.

### 2.5 Cache

Cache payload content below the immutable `cache/namespace.anchor` is disposable and noncanonical. Evicting active payload descendants may affect performance but cannot change the meaning of a completed run or gate. The cache parent, anchor, binding, and parent nonce are canonical namespace bootstrap and cannot be deleted or replaced by ordinary cleanup; cache corruption below the anchor becomes a visible miss. The literal `trash/cache-evictions` entry is reserved from `RetentionPlanId` allocation and names a noncanonical quarantine directory, not a fifth managed parent. Every invocation inspects that literal entry before it manifests active cache payloads, even when the active cache is empty. An absent entry is a valid absence observation. A present entry is opened without following, must be a real same-volume directory, and is completely enumerated without following or mount/volume crossing. Every direct child must match `<invocation-id>.<ordinal>.<manifest-sha256>`, where `invocation-id` is exactly 32 lowercase hexadecimal digits from a fresh 128-bit value, `ordinal` is exactly 16 lowercase hexadecimal digits, and `manifest-sha256` is exactly 64 lowercase hexadecimal digits. The digest is SHA-256 over a length-prefixed `cache-eviction-manifest.v1` frame containing the canonical owner-type byte encoding of the top-level kind, physical and mount/volume identities, link count, regular-file length and content SHA-256 when applicable, and every descendant row ordered by its byte-complete component sequence relative to the top-level root. The frame excludes the active-cache top-level name, quarantine child name, invocation ID, and ordinal so the same physical tree can be recomputed after the move. Every existing child is reopened relative to the held quarantine handle, fully manifested, and required to match the digest in its name before active-cache inspection or success. Its complete tree must contain only same-mount directories and one-link regular files admitted by the payload-manifest rules below. A malformed name, digest disagreement, redirect, mount crossing, unsupported entry, multiply linked regular file, or disagreement between lexical and held topology hard-stops before active-cache mutation or a clean response. Cleanup binds this complete admitted quarantine manifest and, before success, proves that the current tree is exactly the initial admitted tree plus this invocation's digest-matching additions. Trash admission and retention recovery recognize this directory and child grammar only as noncanonical cache quarantine and never parse either as a plan, tombstone, or operation. Freshness and spelling provide collision routing only, never mutation or deletion authority; the manifest digest is restart integrity evidence, not deletion authority.
...
It accepts at most one split-form `--format json` and no other option or positional argument. JSON is the only format in Architecture v1. A successful invocation emits exactly one `lumin.cache-cleanup.v1` object whose only fields are `schemaVersion` and `status`, with `status: "clean"`, followed by the normal single transport newline and with empty stderr. Exit `0` is permitted only after the held cache parent contains exactly the original bound `namespace.anchor` and the complete Section 2.0 namespace proof passes. Detached cache objects may remain under their noncanonical cache-eviction names below the held trash parent. Malformed command input exits `2`; integrity, persistence, or output-delivery failure exits `1`. Failures discovered before transport leave stdout empty and use the ordinary `lumin:` stderr diagnostic. A stdout stream failure may have transferred a nonauthoritative JSON prefix, but no incomplete value is a delivered success object. A `BrokenPipe` stdout failure exits `1` with empty stderr. Any other stdout write or flush failure exits `1` and, when stderr remains writable, emits exactly `lumin: cannot write stdout\n`; a simultaneous stderr failure cannot turn the result into success. Cleanup is an idempotent disposable mutation, not a gate or retention lifecycle mutation: it allocates no operation ID or canonical result record. If output delivery fails after active-cache eviction, repeating the same command is the recovery protocol and must return the same clean response.

Cleanup acquires the exclusive marker-bound lifecycle lock and holds the validated cache/trash parent and anchor handles from the complete four-parent set. It first admits and binds the pre-existing quarantine as specified above. Before the first eviction it then reads a deterministic manifest of every active top-level payload and its complete descendant tree without following redirects or crossing the cache-parent volume/mount. Manifest rows are ordered by byte-complete native component keys and bind the relative component sequence, entry kind, physical and mount/volume identities, link count, and, for a regular one-link file, byte length and SHA-256. Directory identity plus the ordered descendant rows binds the tree; timestamps and traversal order do not. Unsupported entry kinds, multiply linked regular files, redirects, or topology disagreement hard-stop before any payload move. If active payloads exist and `trash/cache-evictions` was absent, cleanup creates it relative to the held trash-parent handle without replacement, immediately durably flushes the held trash parent, and only then reopens, validates, and holds the new same-volume directory; a creation or parent-flush failure permits no payload move. In manifest order, cleanup computes each top-level payload's `cache-eviction-manifest.v1` digest before the move and uses it in the unique `<invocation-id>.<ordinal>.<manifest-sha256>` destination name. It then atomically moves the current winner without replacement from the held cache parent to that name under the held quarantine directory. The atomic move therefore persists the expected pre-move manifest binding with whichever object won the source-name race; it does not depend on a later completion marker or in-memory state surviving. Source and destination parent entries and the complete namespace proof are revalidated around every move, and the cache and quarantine directories are durably flushed after each move.

The move is the command's only mutation of payload bytes. It preserves whichever object won the source-name race until that object is reopened relative to the held quarantine handle. The moved top-level kind, physical identity, mount identity, and complete read-only descendant manifest must equal the initial observation, and its recomputed canonical digest must equal the digest already carried by the destination name. A top-level, nested, or digest mismatch preserves the moved winner in quarantine, leaves every already evicted object quarantined and every later object in the active cache, emits no clean response, and fails without rollback. The command never calls `unlink`, `rmdir`, recursive removal, or an equivalent final disposition on a cache-eviction entry. It therefore cannot turn a destination-name substitution after revalidation into deletion of the substitute.

Physical reclamation of cache-eviction entries is not part of `lumin cache clean`, its success state, or its delivery recovery, and ordinary retention reclamation ignores this disjoint name family. A future store-owned reclamation protocol may dispose of a quarantined object only through an operating-system primitive whose final disposition is bound to the already opened physical object, or through an enforceable isolation boundary that prevents replacement through disposition. A secret or random name, advisory lifecycle lock, second pathname validation, sleep, or scheduler timing is not such authority. If the supported platform supplies neither acceptable mechanism, the noncanonical quarantine remains. After the last move, cleanup performs one final no-follow enumeration proving that the active cache contains only its bound anchor and separately revalidates the complete quarantine against the bound initial manifest plus exactly this invocation's successful additions. The same final validation is mandatory when the active cache was already empty; pre-existing quarantine is never skipped merely because no new move is required. The complete namespace proof still runs around every physical move and again before success.

A process death or delivery failure creates no cache-cleanup operation record. Already moved entries remain noncanonical quarantine; a retry first admits the complete current quarantine by recomputing every named manifest commitment, then validates the current namespace and active cache afresh, leaves every prior admitted quarantine entry untouched, and evicts only the currently observed active payload set under a new invocation ID. An entry moved before the prior process could run its post-move comparison is therefore accepted only when the moved tree still matches the expected pre-move manifest durably carried by its name; a racing substitute remains quarantined but causes an integrity hard-stop instead of becoming prior admitted state. Because payload bytes are disposable and no canonical index points to either location, recovery never interprets a missing or duplicated cache payload as lifecycle deletion truth. A malformed quarantine name, manifest-digest disagreement, redirect, mount crossing, unsupported entry kind, multiply linked regular object, or cache/trash parent-binding disagreement is still an integrity hard-stop rather than a recoverable cache miss, including on an empty-cache retry.

The required concurrency proof has two deterministic substitution barriers. The first stops after the complete initial manifest and before the first top-level move, substitutes another entry at that exact source name, then resumes; post-move validation must report integrity failure and preserve the substitute in quarantine. The second fixture has at least three top-level payloads and stops after one payload is evicted and a child of the next directory is manifested, but before that directory's top-level move. It substitutes that exact child, resumes, and requires the whole directory move followed by a descendant-manifest mismatch: the child substitute remains intact in quarantine, the preceding payload remains evicted, later payloads remain in their original active-cache relative order, the cache/trash parents and anchors retain their physical identities, and the final failed snapshot is asserted. Neither barrier relies on time or scheduler luck. One public-CLI process-death fixture starts a child with at least three ordered payloads, waits for an invocation-nonce-bound test frame emitted only after the first top-level move has passed full manifest comparison and its parent flushes complete and before the second move begins, terminates that child without graceful cleanup, and restarts the same public CLI binary. The retry must admit and preserve the first invocation's quarantine, evict the remaining active payloads under a new invocation ID, finish with an anchor-only active cache and valid combined quarantine, emit exactly one clean response with empty stderr, and leave completed run/gate evidence queryable. A second process-death fixture substitutes the first manifested source name, lets the substitute move under the original manifest digest and become durable, stops the child at an exact frame before post-move comparison, and terminates it. Restart must reject the digest-mismatching quarantine before any further move or success, preserve the substitute and later active payloads, and emit the final failed snapshot. The controller uses exact frames and process termination, never sleep or scheduler luck. Paired public transport fixtures force a partial stdout prefix followed by `BrokenPipe` and by a non-pipe write failure and assert the exact exit/stderr rules above. A paired non-racing run evicts a nested regular-payload tree without recursively deleting it, an empty cache with valid prior quarantine is idempotent, and malformed or digest-mismatching prior quarantine fails before success.
...
Whole `lifecycle.store` migration is a generation-fenced copy-on-write protocol. The marker-bound, one-link `.lumin/lifecycle.lock` and exact four-kind managed-parent/anchor set are outside the replaceable backend and are never themselves replaced. Every ordinary store transaction proves the complete namespace binding, acquires the shared side, opens the current backend only after acquiring it, validates the backend `StoreGeneration` plus global/parent binding, closes the backend handle before releasing the lock, and retains at most a generation token across analysis. No backend handle crosses a transaction boundary. Migration acquires the exclusive side through the same complete proof, so replacement begins only after every old-generation handle has closed; a replaced lock, state directory, parent, or anchor hard-stops before either generation can commit.
...
42. `.lumin` and every alias/descendant are no-follow reserved state; state-directory/lifecycle-lock identities and the exact four-kind managed-parent directory/anchor/nonce set are durably bound and revalidated, so foreign identity, copied/replaced state, lock, parent or payload objects, redirected managed parents, caller writes, validation-to-eviction substitution, or external mutation fail before scan evidence, gate success, or cache-cleanup success can treat them as ordinary state. Cache cleanup performs only atomic no-replace active-cache moves into manifest-committed disjoint noncanonical trash quarantine, rejects an unverified moved substitute after restart, and never performs pathname-based physical deletion.
