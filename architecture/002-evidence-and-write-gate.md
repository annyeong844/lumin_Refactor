...
.lumin/
  ...
  trash/
    ...
    cache-evictions/         # reserved noncanonical cache-quarantine family
      <invocation-id>.<ordinal>  # detached regular file or directory tree
  cache/
    namespace.anchor         # immutable managed-parent bootstrap
...
Cache payload content below the immutable `cache/namespace.anchor` is disposable and noncanonical. Evicting active payload descendants may affect performance but cannot change the meaning of a completed run or gate. The cache parent, anchor, binding, and parent nonce are canonical namespace bootstrap and cannot be deleted or replaced by ordinary cleanup; cache corruption below the anchor becomes a visible miss. The literal `trash/cache-evictions` entry is reserved from `RetentionPlanId` allocation and names a noncanonical quarantine directory, not a fifth managed parent. Every invocation inspects that literal entry before it manifests active cache payloads, even when the active cache is empty. An absent entry is a valid absence observation. A present entry is opened without following, must be a real same-volume directory, and is completely enumerated without following or mount/volume crossing. Every direct child must match `<invocation-id>.<ordinal>`, where `invocation-id` is exactly 32 lowercase hexadecimal digits from a fresh 128-bit value and `ordinal` is exactly 16 lowercase hexadecimal digits, and its complete tree must contain only same-mount directories and one-link regular files admitted by the payload-manifest rules below. A malformed name, redirect, mount crossing, unsupported entry, multiply linked regular file, or disagreement between lexical and held topology hard-stops before active-cache mutation or a clean response. Cleanup binds this initial absence or complete quarantine manifest and, before success, proves that the current tree is exactly the initial admitted tree plus this invocation's successfully moved entries. Trash admission and retention recovery recognize this directory and child grammar only as noncanonical cache quarantine and never parse either as a plan, tombstone, or operation. Freshness and spelling provide collision routing only, never mutation or deletion authority.
