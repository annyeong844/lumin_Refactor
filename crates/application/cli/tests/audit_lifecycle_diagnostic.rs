//! W7 external release children; the driver has no production instrumentation.
#[path = "support/audit_diagnostic.rs"]
mod probe;

#[test]
fn actual_release_children_report_exact_lifecycle_costs_and_durable_recovery()
-> Result<(), Box<dyn std::error::Error>> {
    probe::actual_release_children_report_concrete_pool_and_unchanged_semantics(
        probe::DiagnosticVersion::Lifecycle,
    )
}
