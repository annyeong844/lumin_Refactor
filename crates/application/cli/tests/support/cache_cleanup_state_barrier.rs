...
pub struct CacheCleanupStateBarrier {
    listener: TcpListener,
    environment: &'static str,
    stage: &'static str,
}

impl CacheCleanupStateBarrier {
        ...
        loop {
            match self.listener.accept() {
                Ok((stream, peer)) if peer.ip().is_loopback() => {
                    ...
                    assert_eq!(frame.trim_end(), format!("{} {operation_id}", self.stage));
                    return Ok(Permit { _stream: stream });
                }
                Ok(_) => {
                    return Err(std::io::Error::other(
                        "cache cleanup state barrier accepted a non-loopback peer",
                    )
                    .into());
                }
