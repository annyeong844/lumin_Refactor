...
const CRASH_CORPUS_CASES: &[(&str, &str)] = &[
    ...
    (
        "retention-public-lookup",
        "foundation --store-crash --row retention-public-lookup",
    ),
    (
        "crash-publication",
...
fn validate_corpus_job(jobs: &BTreeMap<String, String>, violations: &mut Vec<String>) {
    ...
    if lines
        ...
        != 33
    {
        violations.push("corpus job must contain exactly 33 reviewed partitions".to_owned());
    }
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (before, after) in [
            ...
            (RELEASE_DIAGNOSTIC_RUN, RELEASE_BENCHMARK_RUN),
        ] {
